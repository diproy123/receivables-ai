...
def create_case(
    ...
    now = datetime.now()
    sla_hours = SLA_TARGETS.get(priority, 72)
...
def auto_create_cases_from_triage(invoice: dict, anomalies: list, triage_result: dict, db: dict, created_by: str = "system") -> list:
        ...
        for ec in existing_cases:
            ...
            if new_aids:
                ...
                if lane == "BLOCK" and has_high and ec["priority"] not in ("critical",):
                    ...
                    ec["sla"]["targetHours"] = SLA_TARGETS["critical"]
                    ec["sla"]["deadline"] = (datetime.fromisoformat(ec["createdAt"]) + timedelta(hours=SLA_TARGETS["critical"])).isoformat()
                    ec["sla"]["warningAt"] = (datetime.fromisoformat(ec["createdAt"]) + timedelta(hours=SLA_TARGETS["critical"] * SLA_WARNING_PCT)).isoformat()
    ...
    contract_viols = [a for a in inv_anomalies if a.get("type") in ("TERMS_VIOLATION", "CONTRACT_PRICE_VIOLATION")]
    price_qty = [a for a in inv_anomalies if a.get("type") in ("PRICE_OVERCHARGE", "QUANTITY_MISMATCH", "AMOUNT_DISCREPANCY", "UNAUTHORIZED_ITEM", "OVERBILLED_VS_RECEIVED", "QTY_RECEIVED_MISMATCH")]
