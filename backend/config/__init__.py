...
# LLM availability — delegates to llm_provider if available, falls back to env check
try:
    from backend.llm_provider import is_llm_available as _is_llm_available
    USE_REAL_API = _is_llm_available()
except ImportError:
...
# ENSEMBLE
# ============================================================
# Model names — read from llm_provider if available, else defaults
try:
    from backend.llm_provider import LLM_PRIMARY_MODEL, LLM_SECONDARY_MODEL
    ENSEMBLE_PRIMARY_MODEL = LLM_PRIMARY_MODEL
    ENSEMBLE_SECONDARY_MODEL = LLM_SECONDARY_MODEL
except ImportError:
    ENSEMBLE_PRIMARY_MODEL = "claude-sonnet-4-20250514"
    ENSEMBLE_SECONDARY_MODEL = "claude-haiku-4-5-20251001"

# Custom model (third ensemble member — fine-tuned or local)
# Set via env vars or policy engine at runtime
# Provider: "anthropic" | "openai_compatible" | "local"
CUSTOM_MODEL_CONFIG = {
    "enabled": bool(os.environ.get("CUSTOM_MODEL_ENABLED", "")),
    "provider": os.environ.get("CUSTOM_MODEL_PROVIDER", "openai_compatible"),
    "model": os.environ.get("CUSTOM_MODEL_NAME", ""),
    "endpoint": os.environ.get("CUSTOM_MODEL_ENDPOINT", ""),     # e.g. http://localhost:8000/v1
    "api_key": os.environ.get("CUSTOM_MODEL_API_KEY", ""),       # for OpenAI-compatible
    "label": os.environ.get("CUSTOM_MODEL_LABEL", "Custom"),     # display name
    "weight": float(os.environ.get("CUSTOM_MODEL_WEIGHT", "1.0")),  # ensemble weight
    "supports_vision": os.environ.get("CUSTOM_MODEL_VISION", "false").lower() == "true",
    "max_tokens": int(os.environ.get("CUSTOM_MODEL_MAX_TOKENS", "4000")),
    "timeout_seconds": int(os.environ.get("CUSTOM_MODEL_TIMEOUT", "30")),
}

# Minimum corrections needed before training data export is useful
FINE_TUNE_MIN_CORRECTIONS = int(os.environ.get("FINE_TUNE_MIN_CORRECTIONS", "50"))
