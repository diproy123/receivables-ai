...
def update_policy(updates: dict) -> dict:
    ...
    BOOL_FIELDS = {k for k, v in DEFAULT_POLICY.items() if isinstance(v, bool)}
    PCT_FIELDS = {k for k, v in DEFAULT_POLICY.items()
                  if isinstance(v, (int, float)) and not isinstance(v, bool)
                  and ('pct' in k or 'confidence' in k or 'risk' in k)}
    DAY_FIELDS = {k for k in DEFAULT_POLICY
                  if k.endswith('_days') or k.endswith('_hours') or k.endswith('_age_days')}

    for key, value in updates.items():
        if key in _active_policy:
            if key == "matching_mode" and value not in VALID_MATCHING_MODES:
                continue
            # Check bool FIRST (before int/float, since bool is subclass of int in Python)
...
def persist_policy():
    """Save active policy to database. Called by server endpoints after their own save."""
    _persist_policy()


def _persist_policy():
    """Save active policy to database for persistence across restarts."""
    try:
        from backend.db import get_db, save_db
        db = get_db()
        db["_policy_state"] = _copy.deepcopy(_active_policy)
        save_db(db)
    except Exception as e:
        print(f"[Policy] Failed to persist policy: {e}")


def _restore_policy():
    """Restore policy from database on startup."""
    try:
        from backend.db import get_db
        db = get_db()
        saved = db.get("_policy_state")
        if saved and isinstance(saved, dict):
            for k, v in saved.items():
                if k in _active_policy:
                    _active_policy[k] = v
            print(f"[Policy] Restored {len(saved)} policy settings from database")
    except Exception as e:
        print(f"[Policy] Could not restore policy (using defaults): {e}")
...
def reset_policy():
    """Reset policy to defaults. Used in testing and /api/reset."""
