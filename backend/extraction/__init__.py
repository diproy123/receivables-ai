...
# PROMPTS
# ============================================================
EXTRACTION_PROMPT = """You are a financial document extraction AI with multilingual capability. You can read documents in any language including English, Chinese (简体中文/繁體中文), Japanese (日本語), Korean (한국어), Hindi (हिन्दी), German, French, Spanish, Portuguese, Arabic, and others.

Extract ALL data from this document into structured JSON. The document may be in any language — extract field values and translate key terms to English where appropriate, but preserve original vendor names, addresses, and item descriptions in their original language alongside English translations.
...
Return ONLY valid JSON (no markdown, no explanation) with this exact structure:
{
  ...
  "vendor_name": "company/vendor name in original language",
  "vendor_name_english": "company/vendor name translated to English (if original is non-English), or same as vendor_name",
  "currency": "USD or INR or EUR or GBP or CNY or JPY or KRW or BRL or SAR or AED — detect from currency symbols ($, Rs, ₹, €, £, ¥, ￥, ₩, R$, ﷼) or text",
  "locale": "detected locale code: en_US, en_GB, en_IN, de_DE, fr_FR, es_ES, pt_BR, zh_CN, ja_JP, ko_KR, ar_SA, or best guess",
  "document_language": "ISO 639-1 code: en, zh, ja, ko, hi, de, fr, es, pt, ar, etc.",
  "subtotal": 10000.00,
  "tax_details": [
    {"type": "GST/CGST/SGST/IGST/VAT/Sales Tax/MwSt/TVA/IVA/增值税/消費税/부가가치세/etc", "rate": 18.0, "amount": 1800.00}
  ],
...
- VENDOR NAMES: Preserve the original-language name in vendor_name. Provide English translation in vendor_name_english.
- LINE ITEMS: Keep descriptions in original language. Add english_description field with translation if non-English.
- TAX TYPES: Use the local tax name (e.g., 增值税, MwSt, TVA, IVA, GST) in tax_details.type. The system understands all standard tax types."""
...
# ============================================================
# LOCALE DETECTION & NUMBER PARSING
# ============================================================
from backend.config import (
    LOCALE_PROFILES, CURRENCY_LOCALE_MAP, DEFAULT_LOCALE
)

def detect_locale(extraction_result: dict) -> str:
    """Detect locale from extraction result. Priority: explicit locale > language+currency > currency > language > default."""
    # 1. Explicit locale from AI extraction
    locale = extraction_result.get("locale", "")
    if locale and locale in LOCALE_PROFILES:
        return locale

    currency = extraction_result.get("currency", "")
    lang = (extraction_result.get("document_language") or "")[:2].lower()

    # 2. Combined: language + currency for disambiguation (e.g., EUR + de → de_DE, EUR + fr → fr_FR)
    if lang and currency:
        for loc_id, loc in LOCALE_PROFILES.items():
            if lang in loc["languages"] and currency in loc["common_currencies"]:
                return loc_id
        # No exact match — try currency-first with language as secondary signal
        # e.g., EUR + en → no en_XX has EUR, so fall to currency-based locale
        if currency in CURRENCY_LOCALE_MAP:
            return CURRENCY_LOCALE_MAP[currency]

    # 3. Infer from document_language alone
    if lang:
        for loc_id, loc in LOCALE_PROFILES.items():
            if lang in loc["languages"]:
                return loc_id

    # 4. Infer from currency
    if currency in CURRENCY_LOCALE_MAP:
        return CURRENCY_LOCALE_MAP[currency]
...
def get_locale_profile(locale_code: str) -> dict:
    """Get the full locale profile, falling back to en_US."""
    return LOCALE_PROFILES.get(locale_code, LOCALE_PROFILES[DEFAULT_LOCALE])


def parse_locale_number(value, locale_code: str = None) -> float:
    """Parse a number string respecting locale decimal/thousands conventions.
    Handles: 1,500.00 (en), 1.500,00 (de/es/pt/fr), 1 500,00 (fr alt), ￥15,000 (CJK)."""
    ...
    # Auto-detect format if locale not given
    profile = get_locale_profile(locale_code) if locale_code else None

    if profile and profile["decimal_sep"] == ",":
        # European style: 1.500,00 → 1500.00
        cleaned = cleaned.replace(' ', '').replace('.', '').replace(',', '.')
    else:
        # US/CJK style: 1,500.00 → 1500.00, or auto-detect
        if ',' in cleaned and '.' in cleaned:
            ...
            # Could be thousands (1,500) or decimal (1,50)
            parts = cleaned.split(',')
            if len(parts) == 2 and len(parts[1]) <= 2:
                # Likely decimal: 1,50 → 1.50
                cleaned = cleaned.replace(',', '.')
            else:
                # Likely thousands: 1,500 or 1,500,000
                cleaned = cleaned.replace(',', '')
        else:
            cleaned = cleaned.replace(',', '')
...
def parse_locale_date(date_str: str, locale_code: str = None) -> str:
    """Parse a date string using locale-aware formats, return YYYY-MM-DD.
    Returns original string if already YYYY-MM-DD or unparseable."""
    ...
    if len(date_str) >= 10 and date_str[4] == '-' and date_str[7] == '-':
        try:
            _dt.strptime(date_str[:10], "%Y-%m-%d")
            return date_str[:10]
        except ValueError:
            pass

    # Try locale-specific formats first, then all known formats
    all_formats = []
    if locale_code:
        profile = get_locale_profile(locale_code)
        all_formats.extend(profile.get("date_formats", []))

    # Add common global formats as fallback
    all_formats.extend([
        "%Y-%m-%d", "%d/%m/%Y", "%m/%d/%Y", "%d-%m-%Y", "%m-%d-%Y",
        "%d.%m.%Y", "%d.%m.%y", "%Y/%m/%d",
        "%Y年%m月%d日", "%d %B %Y", "%B %d, %Y", "%b %d, %Y",
        "%d %b %Y", "%d-%b-%Y", "%d %b, %Y",
    ])
...
def normalize_extraction_locale(result: dict) -> dict:
    """Post-process extraction result to normalize numbers and dates per detected locale.
    This ensures all downstream code receives consistent formats regardless of document locale."""
    locale_code = detect_locale(result)
    result["_detected_locale"] = locale_code

    # Normalize numeric fields
    for field in ("subtotal", "total_amount"):
        val = result.get(field)
        if isinstance(val, str):
            result[field] = parse_locale_number(val, locale_code)
    ...
    for li in (result.get("line_items") or []):
        for nf in ("quantity", "unit_price", "total"):
            val = li.get(nf)
            if isinstance(val, str):
                li[nf] = parse_locale_number(val, locale_code)

    # Normalize tax detail numbers
    for td in (result.get("tax_details") or []):
        for nf in ("rate", "amount"):
            val = td.get(nf)
            if isinstance(val, str):
                td[nf] = parse_locale_number(val, locale_code)

    # Normalize date fields
    for df in ("issue_date", "due_date", "delivery_date", "received_date"):
        val = result.get(df)
        if val and isinstance(val, str):
            result[df] = parse_locale_date(val, locale_code)
...
# ENSEMBLE MERGE
# ============================================================
def _ensemble_merge_multi(results: list, labels: list, weights: dict) -> tuple:
    """
    Merge N extraction results with majority voting and weighted confidence.

    Strategy:
      - Numeric fields: weighted median (robust to one outlier)
      - String fields:  majority vote (2/3 agree = high confidence)
      - Line items:     take from highest-weight model that has them
      - Tax details:    take from highest-weight model that has them

    Returns (merged, field_confidence, metadata) — same format as _ensemble_merge.
    """
    n = len(results)
    merged, field_conf = {}, {}
    ...
    for f in _NUMERIC_FIELDS:
        ...
        for i, r in enumerate(results):
            v = r.get(f)
            if v is not None:
                w = weights.get(labels[i], 1.0)
                values.append((float(v), w, labels[i]))

        if not values:
            continue
        elif len(values) == 1:
            merged[f] = values[0][0]
            field_conf[f] = {"status": "single_source", "confidence": "medium",
                             "source": values[0][2]}
        else:
            # Check consensus
            all_close = all(_numeric_close(values[0][0], v[0], 1.0) for v in values[1:])
            mostly_close = sum(1 for v in values[1:] if _numeric_close(values[0][0], v[0], 5.0)) >= len(values) - 2

            if all_close:
                # Weighted average when all agree
                total_w = sum(v[1] for v in values)
                merged[f] = round(sum(v[0] * v[1] for v in values) / total_w, 2) if total_w > 0 else values[0][0]
                field_conf[f] = {"status": "agreed", "confidence": "high",
                                 "values": {v[2]: v[0] for v in values}}
                agreements += 1
            elif mostly_close:
                # Use value from highest-weight model
                best = max(values, key=lambda v: v[1])
                merged[f] = best[0]
                field_conf[f] = {"status": "near_match", "confidence": "medium",
                                 "values": {v[2]: v[0] for v in values}, "selected": best[2]}
                agreements += 1
            else:
                # Disputed — majority vote or highest weight
                # Group by closeness to find majority cluster
                best = max(values, key=lambda v: v[1])
                merged[f] = best[0]
                field_conf[f] = {"status": "disputed", "confidence": "low",
                                 "values": {v[2]: v[0] for v in values}, "selected": best[2]}
                disputes += 1
    ...
    for f in _string_fields:
        ...
        for i, r in enumerate(results):
            ...
            if v:
                w = weights.get(labels[i], 1.0)
                values.append((str(v), w, labels[i]))
        ...
        if not values:
            ...
            # Group by similarity — find majority cluster
            groups = []  # list of (canonical_value, total_weight, members)
            for val, w, lbl in values:
                matched = False
                for g in groups:
                    if _string_match(val, g[0]) in ("exact", "close"):
                        g[1] += w
                        g[2].append(lbl)
                        matched = True
                        break
                if not matched:
                    groups.append([val, w, [lbl]])

            # Pick the majority (most weight) group
            best_group = max(groups, key=lambda g: g[1])
            majority_count = len(best_group[2])

            merged[f] = best_group[0]
            if majority_count == n:
                field_conf[f] = {"status": "agreed", "confidence": "high"}
                agreements += 1
            elif majority_count >= 2:
                field_conf[f] = {"status": "majority", "confidence": "high",
                                 "agreed_by": best_group[2],
                                 "dissented": [lbl for _, _, lbl in values if lbl not in best_group[2]]}
                agreements += 1
            else:
                field_conf[f] = {"status": "disputed", "confidence": "medium",
                                 "values": {v[2]: v[0] for v in values}}
                disputes += 1

    # Line items: take from highest-weight model with line items
    li_candidates = [(r.get("line_items") or [], weights.get(labels[i], 1.0), labels[i])
                     for i, r in enumerate(results) if r.get("line_items")]
    if li_candidates:
        best_li = max(li_candidates, key=lambda x: (len(x[0]), x[1]))
        merged["line_items"] = best_li[0]
        field_conf["line_items"] = {"status": "selected", "source": best_li[2],
                                     "models_with_items": len(li_candidates)}
    else:
        merged["line_items"] = []
        field_conf["line_items"] = {"status": "all_empty"}

    # Tax details: similarly
    tax_candidates = [(r.get("tax_details") or [], weights.get(labels[i], 1.0), labels[i])
                      for i, r in enumerate(results) if r.get("tax_details")]
    if tax_candidates:
        best_tax = max(tax_candidates, key=lambda x: (len(x[0]), x[1]))
        merged["tax_details"] = best_tax[0]
        field_conf["tax_details"] = {"status": "selected", "source": best_tax[2]}
    else:
        merged["tax_details"] = []
        field_conf["tax_details"] = {"status": "both_empty"}

    # Primary-only fields: take from highest-priority model that has them
    ...
    meta = {
        "models_used": [r.get("_model", "unknown") for r in results],
        "model_labels": labels,
        "model_weights": weights,
        "model_latencies": {labels[i]: results[i].get("_latency_ms", 0) for i in range(n)},
        "fields_compared": total_compared, "fields_agreed": agreements,
        "fields_disputed": disputes, "agreement_rate": agreement_rate,
        "ensemble_mode": f"{n}-model",
...
async def extract_with_claude(file_path: str, file_name: str, media_type: str,
    ...
    custom_result = results[2] if custom_enabled and len(results) > 2 else None
    ...
    if isinstance(custom_result, Exception):
        custom_result = {"_error": str(custom_result), "_model": "custom"}

    primary_ok = "_error" not in primary_result
    secondary_ok = "_error" not in secondary_result
    custom_ok = custom_result is not None and "_error" not in custom_result
    ...
    # Get vendor-specific weights for model trust
    vendor_name = primary_result.get("vendor_name") or secondary_result.get("vendor_name", "")
    model_weights = get_model_weights(vendor_name)

    if custom_ok:
        # 3-model merge: majority vote for strings, weighted median for numbers
        merged, field_conf, meta = _ensemble_merge_multi(
            [primary_result, secondary_result, custom_result],
            ["primary", "secondary", "custom"],
            model_weights
        )
    else:
        merged, field_conf, meta = _ensemble_merge(primary_result, secondary_result)
        if custom_enabled and not custom_ok:
