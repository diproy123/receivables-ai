...
class TFIDFEmbedder:
    ...
    def __init__(self):
        ...
        self._fitted = False
        self._last_corpus_size = 0  # Track corpus size to know when to refit
    ...
    def embed(self, text: str) -> list:
        ...
        # Only refit when corpus size has changed significantly (>20% growth or first time)
        current_size = len(store.chunks)
        needs_refit = (not self._fitted
            or current_size == 0
            or abs(current_size - self._last_corpus_size) > max(1, self._last_corpus_size * 0.2))

        if needs_refit:
            corpus = [c["text"] for c in store.chunks] + [text]
            self.fit(corpus)
            self._last_corpus_size = current_size
...
async def ingest_anomalies_batch(anomalies: list):
    """Add multiple anomalies to the vector store with a single save."""
    if not anomalies:
        return
    chunks = [chunk_anomaly(a) for a in anomalies]
    texts = [c["text"] for c in chunks]
    embeddings = await get_embeddings_batch(texts)
    for chunk, embedding in zip(chunks, embeddings):
        store.add(chunk["id"], chunk["text"], embedding, chunk["metadata"])
...
async def on_anomalies_detected_batch(anomalies: list):
    """Call after multiple anomalies detected. Batch indexes into RAG."""
    await ingest_anomalies_batch(anomalies)
