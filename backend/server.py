...
TRIAGE_AUTO_APPROVE_MAX_RISK = float(os.environ.get("TRIAGE_AUTO_APPROVE_MAX_RISK", "50"))
...
async def get_dashboard():
    ...
    return {"total_ar": round(tar, 2), "unpaid_count": len(unpaid), "total_documents": len(ad),
        ...
        "savings_discovered": round(
            sum(_n(a.get("amount_at_risk")) for a in db.get("anomalies", []) if _n(a.get("amount_at_risk")) > 0), 2),
        "savings_breakdown": {
            "overcharges": round(sum(_n(a.get("amount_at_risk")) for a in db.get("anomalies", [])
                if a.get("type") in ("PRICE_OVERCHARGE", "AMOUNT_DISCREPANCY", "CONTRACT_PRICE_VIOLATION") and _n(a.get("amount_at_risk")) > 0), 2),
            "duplicates_prevented": round(sum(_n(a.get("amount_at_risk")) for a in db.get("anomalies", [])
                if a.get("type") == "DUPLICATE_INVOICE" and _n(a.get("amount_at_risk")) > 0), 2),
            "contract_violations": round(sum(_n(a.get("amount_at_risk")) for a in db.get("anomalies", [])
                if a.get("type") == "TERMS_VIOLATION" and _n(a.get("amount_at_risk")) > 0), 2),
            "unauthorized_items": round(sum(_n(a.get("amount_at_risk")) for a in db.get("anomalies", [])
                if a.get("type") in ("UNAUTHORIZED_ITEM", "QUANTITY_OVERCHARGE") and _n(a.get("amount_at_risk")) > 0), 2),
...
async def seed_demo():
    """Seed the database with demo data. Only works if DB is empty or force=true."""
    db = get_db()
    total = sum(len(v) for v in db.values() if isinstance(v, list))
    if total > 0:
