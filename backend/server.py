...
try:
    from backend.rag_engine import (on_document_uploaded, on_anomaly_detected, on_document_edited,
        get_extraction_context, get_anomaly_context, get_rag_stats, reset_rag)
    RAG_ENABLED = True
except ImportError:
    try:
...
async def upload_document(file: UploadFile = File(...), document_type: str = Form("auto")):
    ...
    try:
        ...
        for anom in new_anomalies:
            await on_anomaly_detected(anom)
    except Exception as e:
...
async def edit_document_fields(did: str, fields: str = Form(...)):
    ...
    if "subtotal" in changes or "taxDetails" in changes:
    ...
    # Re-run matching + anomaly detection for invoices
    if doc["type"] == "invoice":
        # Remove old matches and anomalies for this invoice
        db["matches"] = [m for m in db["matches"] if m.get("invoiceId") != did]
        db["anomalies"] = [a for a in db["anomalies"] if a.get("invoiceId") != did]
