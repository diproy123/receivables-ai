...
async def extract_with_claude(file_path: str, file_name: str, media_type: str, vendor_hint: str = "", doc_type_hint: str = "") -> dict:
    ...
    try:
        print(f"[Extract] Calling Claude API for '{file_name}' (media_type={media_type}, b64_len={len(b64_data)})")
        ...
        for model in models:
            try:
                ...
                print(f"[Extract] Model '{model}' succeeded")
                break
            except anthropic.NotFoundError:
                print(f"[Extract] Model '{model}' not found, trying next...")
                continue
            except anthropic.BadRequestError as e:
                # If document type not supported, retry as image (convert PDF page to image approach)
                if "document" in str(e).lower() and media_type == "application/pdf":
                    print(f"[Extract] Document type not supported by '{model}', retrying as image...")
                    content_block = {"type": "image", "source": {"type": "base64", "media_type": "image/png", "data": b64_data}}
                    try:
                        msg = client.messages.create(model=model, max_tokens=4000,
                            messages=[{"role": "user", "content": [content_block, {"type": "text", "text": prompt}]}])
                        print(f"[Extract] Image fallback succeeded with '{model}'")
                        break
                    except Exception as e2:
                        print(f"[Extract] Image fallback also failed: {e2}")
                        continue
                else:
                    raise
        if not msg:
        ...
        print(f"[Extract] Raw response (first 300 chars): {text[:300]}")
        ...
        print(f"[Extract] Success: type={result.get('document_type')}, vendor={result.get('vendor_name')}, amount={result.get('total_amount')}")
        return result
    except json.JSONDecodeError as e:
        print(f"[Extract] JSON PARSE ERROR for '{file_name}': {e}")
        print(f"[Extract] Text that failed to parse: {text[:500] if 'text' in dir() else 'N/A'}")
        return await mock_extraction(file_name)
    except anthropic.BadRequestError as e:
        print(f"[Extract] API BAD REQUEST for '{file_name}': {e}")
        return await mock_extraction(file_name)
    except anthropic.AuthenticationError as e:
        print(f"[Extract] API AUTH ERROR: {e}")
        return await mock_extraction(file_name)
    except anthropic.NotFoundError as e:
        print(f"[Extract] API MODEL NOT FOUND: {e} — try changing model name")
        return await mock_extraction(file_name)
    except Exception as e:
        print(f"[Extract] UNEXPECTED ERROR for '{file_name}': {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
        return await mock_extraction(file_name)
    ...
    try:
        ...
        for model in models:
            try:
                ...
                continue
        if not msg:
            raise RuntimeError("All models failed for anomaly detection")
        text = msg.content[0].text.strip()
        if text.startswith("```"):
            text = text.split("\n", 1)[1]
