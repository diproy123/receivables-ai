...
async def extract_with_claude(file_path: str, file_name: str, media_type: str, vendor_hint: str = "", doc_type_hint: str = "") -> dict:
    if not USE_REAL_API:
        # No API key configured — return extraction failure so user can manually index
    ...
    try:
        ...
        print(f"[Extract] Raw text was: {text[:500] if 'text' in dir() else 'N/A'}")
        return {"_source": "claude_api_parse_error", "_extraction_failed": True,
                "document_type": doc_type_hint or "invoice", "vendor_name": "Unknown",
                "total_amount": 0, "extraction_confidence": 0,
                "_error": f"AI extraction succeeded but response could not be parsed. Use Manual Entry or re-upload."}
    except Exception as e:
        print(f"[Extract] API ERROR for '{file_name}': {type(e).__name__}: {e}")
        import traceback; traceback.print_exc()
        return {"_source": "claude_api_error", "_extraction_failed": True,
                "document_type": doc_type_hint or "invoice", "vendor_name": "Unknown",
                "total_amount": 0, "extraction_confidence": 0,
                "_error": f"AI extraction failed: {type(e).__name__}: {str(e)}. Use Manual Entry to index this document."}
...
async def health():
    return {"status": "ok", "product": "AuditLens",
        "claude_api": "connected" if USE_REAL_API else "no_api_key", "version": "2.5.0",
