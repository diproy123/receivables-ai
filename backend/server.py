...
async def register(request: Request):
    ...
    # First user gets cfo role automatically
    if not users:
        role = "cfo"
...
# ============================================================
def triage_invoice(invoice: dict, anomalies: list, db: dict, role: str = None) -> dict:
...
async def update_policy_endpoint(request: Request):
    """Update policy fields. Accepts partial updates — only send fields to change."""
    try:
        body = await request.json()
    except:
        raise HTTPException(400, "Invalid JSON")
...
async def apply_policy_preset(preset_name: str):
    """Apply a named policy preset (manufacturing, services, enterprise_default, strict_audit)."""
