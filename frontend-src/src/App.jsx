          ...
          <div className="card w-full max-w-[720px] max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-gradient-to-r from-slate-800 to-slate-700 text-white p-6 rounded-t-2xl">
              <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => {
                    const el = document.getElementById('intel-report-content');
                    if (!el) return;
                    const w = window.open('', '_blank', 'width=800,height=900');
                    w.document.write(`<html><head><title>Contract Intelligence Briefing - ${intelReport.period}</title>
                      <style>body{font-family:system-ui,sans-serif;padding:40px;max-width:720px;margin:0 auto;color:#1e293b}
                      h1{font-size:22px;margin-bottom:4px} .sub{color:#64748b;font-size:14px;margin-bottom:20px}
                      .summary{background:#f1f5f9;padding:14px 18px;border-radius:10px;font-size:14px;margin-bottom:24px;line-height:1.5}
                      h3{font-size:13px;text-transform:uppercase;letter-spacing:.05em;margin:20px 0 10px}
                      .card{border:1px solid #e2e8f0;border-radius:10px;padding:12px 16px;margin-bottom:8px}
                      .card.action{border-left:4px solid #f59e0b;background:#fffbeb}
                      .card.critical{border-left:4px solid #ef4444;background:#fef2f2}
                      .card.clause{border-left:3px solid #f97316;background:#fff7ed}
                      .pill{display:inline-block;padding:6px 16px;border-radius:10px;margin-right:8px;font-size:13px;margin-bottom:6px}
                      ...
                      .action-text{color:#4f46e5;font-size:13px;margin-top:6px} .owner{color:#94a3b8;font-size:12px;float:right}
                      .clause-type{font-weight:600;font-size:12px;text-transform:uppercase;color:#64748b;margin-bottom:2px}
                      @media print{body{padding:20px} .no-print{display:none}}</style></head><body>
                      <h1>Contract Intelligence Briefing</h1>
                      <div class="sub">${intelReport.period}</div>
                      ...
                      Generated by AuditLens · ${new Date().toLocaleDateString()}</div>
                      </body></html>`);
                    w.document.close();
                  ...
                  <button onClick={() => {
                    ...
                    const lines = [`Contract Intelligence Briefing — ${intelReport.period}\n`, intelReport.summary_line, ''];
                    if (exp?.count > 0) {
                    ...
                    if (risk) {
                      lines.push(`PORTFOLIO: ${risk.healthy} Healthy, ${risk.warning} Warning, ${risk.critical} Critical, ${risk.high_risk_clauses_total} high-risk clauses`);
                      if ((risk.high_risk_clause_details || []).length > 0) {
                        lines.push('  High-risk clauses:');
                        (risk.high_risk_clause_details || []).forEach(cl => lines.push(`  • ${cl.vendor} (${cl.contract}): ${cl.clause_type} — ${cl.summary}`));
                      }
                      lines.push('');
                    }
                    lines.push(`Generated by AuditLens · ${new Date().toLocaleDateString()}`);
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    {/* Section 3: Portfolio Health — with drill-down */}
                    {risk && (
                      <div>
                        ...
                        {/* Drill-down: Contracts needing attention */}
                        {(risk.worst_contracts || []).filter(w => w.health_level !== 'good').length > 0 && (
                          <div className="mb-3">
                            <div className="text-xs font-semibold text-slate-500 uppercase mb-2">Contracts Needing Attention</div>
                            <div className="space-y-1.5">
                              {(risk.worst_contracts || []).filter(w => w.health_level !== 'good').map((w, i) => (
                                <div key={i} className={cn("flex items-center justify-between p-3 rounded-xl border",
                                  w.health_level === 'critical' ? 'bg-red-50 border-red-100' : 'bg-amber-50 border-amber-100')}>
                                  <div>
                                    <span className="text-sm font-semibold text-slate-800">{w.vendor}</span>
                                    <span className="text-xs text-slate-500 ml-2">({w.contract})</span>
                                  </div>
                                  <div className="flex items-center gap-3">
                                    {w.high_risk_clauses > 0 && <span className="text-xs text-orange-600 font-semibold">{w.high_risk_clauses} risky clause{w.high_risk_clauses > 1 ? 's' : ''}</span>}
                                    <div className={cn("text-xs font-bold px-2 py-1 rounded-full",
                                      w.health_level === 'critical' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700')}>
                        ...
                        {/* Drill-down: High-risk clause details */}
                        {(risk.high_risk_clause_details || []).length > 0 && (
                          <div>
                            <div className="text-xs font-semibold text-slate-500 uppercase mb-2">High-Risk Clause Details</div>
                            <div className="space-y-1.5">
                              {(risk.high_risk_clause_details || []).map((cl, i) => (
                                <div key={i} className="p-3 rounded-xl border border-orange-200 bg-orange-50/50 border-l-4 border-l-orange-400">
                                  <div className="flex items-center justify-between mb-1">
                                    <span className="text-[11px] font-bold text-slate-500 uppercase">{cl.clause_type}</span>
                                    <span className="text-xs text-slate-400">{cl.vendor} · {cl.contract}</span>
                                  </div>
