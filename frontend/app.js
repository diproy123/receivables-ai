...
function loginScreen(){
  ...
  return`<div style="min-height:100vh;display:flex;background:linear-gradient(165deg,#0f1729 0%,#1a2744 50%,#0f1729 100%)">
<div style="flex:1;display:flex;flex-direction:column;justify-content:center;padding:60px;max-width:520px">
<div style="position:absolute;top:-200px;left:-100px;width:500px;height:500px;border-radius:50%;background:radial-gradient(circle,rgba(26,86,219,.1) 0%,transparent 60%);pointer-events:none"></div>
<div style="position:relative;z-index:1">
<div style="display:flex;align-items:center;gap:12px;margin-bottom:48px"><div style="width:40px;height:40px;border-radius:10px;background:linear-gradient(135deg,#dc2626,#ef4444);display:flex;align-items:center;justify-content:center;box-shadow:0 4px 16px rgba(220,38,38,.3)"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round"><path d="M12 2L2 7v10l10 5 10-5V7L12 2z"/><path d="M12 22V12"/><path d="M2 7l10 5 10-5"/></svg></div><span style="font-size:20px;font-weight:800;color:white;letter-spacing:-.02em">AuditLens</span></div>
<h1 style="font-size:32px;font-weight:800;color:white;margin-bottom:8px;letter-spacing:-.03em">${m==='login'?'Welcome back':'Create your account'}</h1>
<p style="font-size:15px;color:rgba(255,255,255,.45);margin-bottom:32px">${m==='login'?'Sign in to your AP dashboard':'Start auditing invoices in minutes'}</p>
<div style="display:flex;margin-bottom:28px;border-radius:10px;overflow:hidden;border:1px solid rgba(255,255,255,.1)"><button style="flex:1;padding:11px;border:none;cursor:pointer;font-weight:600;font-size:13px;font-family:var(--ft);transition:all .2s;${m==='login'?'background:rgba(26,86,219,.2);color:#60a5fa':'background:rgba(255,255,255,.03);color:rgba(255,255,255,.4)'}" onclick="S.authMode='login';S.authError=null;R()">Sign In</button><button style="flex:1;padding:11px;border:none;cursor:pointer;font-weight:600;font-size:13px;font-family:var(--ft);transition:all .2s;${m==='register'?'background:rgba(26,86,219,.2);color:#60a5fa':'background:rgba(255,255,255,.03);color:rgba(255,255,255,.4)'}" onclick="S.authMode='register';S.authError=null;R()">Register</button></div>
${S.authError?`<div style="padding:11px 14px;background:rgba(190,18,60,.12);color:#fca5a5;border-radius:10px;font-size:13px;font-weight:500;margin-bottom:16px;border:1px solid rgba(190,18,60,.2)">${E(S.authError)}</div>`:''}
${m==='register'?`<div style="margin-bottom:16px"><label style="font-size:12px;font-weight:600;color:rgba(255,255,255,.5);display:block;margin-bottom:6px;text-transform:uppercase;letter-spacing:.06em">Full Name</label><input id="auth-name" type="text" placeholder="Your name" style="width:100%;padding:12px 14px;border:1px solid rgba(255,255,255,.1);border-radius:10px;font-size:14px;font-family:var(--ft);background:rgba(255,255,255,.04);color:white;outline:none;box-sizing:border-box;transition:border-color .2s" onfocus="this.style.borderColor='rgba(96,165,250,.4)'" onblur="this.style.borderColor='rgba(255,255,255,.1)'"></div>`:''}
<div style="margin-bottom:16px"><label style="font-size:12px;font-weight:600;color:rgba(255,255,255,.5);display:block;margin-bottom:6px;text-transform:uppercase;letter-spacing:.06em">Email</label><input id="auth-email" type="email" placeholder="name@company.com" style="width:100%;padding:12px 14px;border:1px solid rgba(255,255,255,.1);border-radius:10px;font-size:14px;font-family:var(--ft);background:rgba(255,255,255,.04);color:white;outline:none;box-sizing:border-box;transition:border-color .2s" onfocus="this.style.borderColor='rgba(96,165,250,.4)'" onblur="this.style.borderColor='rgba(255,255,255,.1)'"></div>
<div style="margin-bottom:16px"><label style="font-size:12px;font-weight:600;color:rgba(255,255,255,.5);display:block;margin-bottom:6px;text-transform:uppercase;letter-spacing:.06em">Password</label><input id="auth-password" type="password" placeholder="\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022" style="width:100%;padding:12px 14px;border:1px solid rgba(255,255,255,.1);border-radius:10px;font-size:14px;font-family:var(--ft);background:rgba(255,255,255,.04);color:white;outline:none;box-sizing:border-box;transition:border-color .2s" onfocus="this.style.borderColor='rgba(96,165,250,.4)'" onblur="this.style.borderColor='rgba(255,255,255,.1)'"></div>
${m==='register'?`<div style="margin-bottom:24px"><label style="font-size:12px;font-weight:600;color:rgba(255,255,255,.5);display:block;margin-bottom:6px;text-transform:uppercase;letter-spacing:.06em">Role</label><select id="auth-role" style="width:100%;padding:12px 14px;border:1px solid rgba(255,255,255,.1);border-radius:10px;font-size:14px;font-family:var(--ft);background:#1a2744;color:white;outline:none;-webkit-appearance:none;appearance:none;background-image:url('data:image/svg+xml;utf8,<svg fill=\\"white\\" viewBox=\\"0 0 24 24\\" xmlns=\\"http://www.w3.org/2000/svg\\"><path d=\\"M7 10l5 5 5-5z\\"/></svg>');background-repeat:no-repeat;background-position:right 12px center;background-size:16px"><option value="analyst" style="background:#1a2744;color:white">AP Analyst</option><option value="manager" style="background:#1a2744;color:white">AP Manager</option><option value="vp" style="background:#1a2744;color:white">VP Finance</option><option value="cfo" style="background:#1a2744;color:white">CFO</option></select><div style="font-size:11px;color:rgba(255,255,255,.3);margin-top:6px">First user is automatically promoted to CFO</div></div>`:''}
<button id="auth-submit" class="bt bt-p" style="width:100%;padding:13px;font-size:15px;font-weight:700;margin-top:4px" ${S.authLoading?'disabled':''}>${S.authLoading?'Authenticating...':m==='login'?'Sign In \u2192':'Create Account \u2192'}</button>
</div></div>
<div style="flex:1;display:flex;align-items:center;justify-content:center;padding:40px;position:relative">
<div style="max-width:400px;text-align:center;position:relative;z-index:1">
<div style="font-size:48px;margin-bottom:20px;animation:float 4s ease-in-out infinite">\uD83D\uDEE1\uFE0F</div>
<div style="font-size:22px;font-weight:700;color:white;margin-bottom:12px">Enterprise AP Automation</div>
<div style="font-size:14px;color:rgba(255,255,255,.4);line-height:1.7;margin-bottom:32px">Three-way matching, anomaly detection, delegation of authority, and configurable policies \u2014 all powered by AI.</div>
...
].map(f=>`<div style="display:flex;align-items:center;gap:10px;padding:12px 16px;background:rgba(255,255,255,.04);border-radius:10px;border:1px solid rgba(255,255,255,.06)"><span style="font-size:16px">${f.i}</span><span style="font-size:13px;color:rgba(255,255,255,.6);font-weight:500">${f.t}</span></div>`).join('')}
</div></div></div></div>`}
...
<div style="min-height:100vh;background:linear-gradient(165deg,#0f1729 0%,#1a2744 40%,#1e3a5f 70%,#0f1729 100%);position:relative;overflow:hidden">
<div style="position:absolute;top:-300px;right:-200px;width:800px;height:800px;border-radius:50%;background:radial-gradient(circle,rgba(26,86,219,.15) 0%,transparent 60%);pointer-events:none"></div>
<div style="position:absolute;bottom:-200px;left:-100px;width:600px;height:600px;border-radius:50%;background:radial-gradient(circle,rgba(220,38,38,.08) 0%,transparent 60%);pointer-events:none"></div>
<nav style="padding:24px 56px;display:flex;align-items:center;justify-content:space-between;position:relative;z-index:10">
<div style="display:flex;align-items:center;gap:14px"><div style="width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,#dc2626,#ef4444);display:flex;align-items:center;justify-content:center;box-shadow:0 4px 16px rgba(220,38,38,.3)"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round"><path d="M12 2L2 7v10l10 5 10-5V7L12 2z"/><path d="M12 22V12"/><path d="M2 7l10 5 10-5"/></svg></div><span style="font-size:22px;font-weight:800;letter-spacing:-.03em;color:white">AuditLens</span></div>
<div style="display:flex;gap:10px"><button class="bt" style="background:rgba(255,255,255,.08);color:rgba(255,255,255,.8);border:1px solid rgba(255,255,255,.12);backdrop-filter:blur(8px)" data-go>Sign In</button><button class="bt bt-p" data-go>Get Started \u2192</button></div></nav>

<section class="fu" style="padding:100px 56px 80px;max-width:960px;margin:0 auto;text-align:center;position:relative;z-index:10">
<div style="display:inline-flex;align-items:center;gap:8px;padding:6px 16px;border-radius:24px;background:rgba(26,86,219,.12);border:1px solid rgba(26,86,219,.2);margin-bottom:28px;font-size:13px;font-weight:600;color:#60a5fa;backdrop-filter:blur(4px)"><span style="width:6px;height:6px;border-radius:50%;background:#60a5fa;animation:pulse 2s infinite"></span> AI-Powered Accounts Payable Intelligence</div>
<h1 style="font-size:56px;font-weight:800;letter-spacing:-.045em;line-height:1.08;margin-bottom:24px;color:white">Every invoice audited.<br><span style="background:linear-gradient(135deg,#60a5fa,#38bdf8);-webkit-background-clip:text;-webkit-text-fill-color:transparent">Every dollar accounted.</span></h1>
<p style="font-size:19px;color:rgba(255,255,255,.6);line-height:1.7;max-width:620px;margin:0 auto 44px;font-weight:400">Three-way matching, anomaly detection, and delegation of authority \u2014 AuditLens brings enterprise AP automation to every finance team.</p>
<div style="display:flex;gap:14px;justify-content:center"><button class="bt bt-p" style="padding:16px 36px;font-size:16px;font-weight:700" data-go>Start Auditing \u2192</button><button class="bt" style="padding:16px 36px;font-size:16px;background:rgba(255,255,255,.06);color:white;border:1px solid rgba(255,255,255,.15)" onclick="window.open('mailto:sales@auditlens.ai')">Talk to Sales</button></div></section>

<section style="padding:0 56px 80px;max-width:1000px;margin:0 auto;position:relative;z-index:10"><div style="display:grid;grid-template-columns:repeat(4,1fr);gap:2px;background:rgba(255,255,255,.06);border-radius:16px;overflow:hidden;border:1px solid rgba(255,255,255,.08)">
${[{v:'15+',l:'Anomaly Types'},{v:'3-Way',l:'PO-GRN-Invoice Matching'},{v:'4-Tier',l:'Approval Authority'},{v:'<2s',l:'Per Document'}].map(m=>`<div style="background:rgba(15,23,41,.6);padding:28px 20px;text-align:center;backdrop-filter:blur(8px)"><div style="font-size:32px;font-weight:800;color:white;font-family:var(--mn);letter-spacing:-.02em">${m.v}</div><div style="font-size:12px;color:rgba(255,255,255,.45);font-weight:500;margin-top:6px;text-transform:uppercase;letter-spacing:.06em">${m.l}</div></div>`).join('')}
</div></section>

<section style="padding:20px 56px 100px;max-width:1060px;margin:0 auto;position:relative;z-index:10"><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px">${[
...
].map((f,i)=>`<div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:16px;padding:28px;transition:all .3s;animation:fadeUp .5s cubic-bezier(.22,1,.36,1) ${i*0.08}s both" onmouseover="this.style.background='rgba(255,255,255,.07)';this.style.borderColor='rgba(255,255,255,.15)'" onmouseout="this.style.background='rgba(255,255,255,.04)';this.style.borderColor='rgba(255,255,255,.08)'"><div style="font-size:28px;margin-bottom:14px">${f.i}</div><div style="font-size:16px;font-weight:700;margin-bottom:8px;color:white">${f.t}</div><div style="font-size:14px;color:rgba(255,255,255,.5);line-height:1.65">${f.d}</div></div>`).join('')}</div></section>

<section style="padding:40px 56px 80px;max-width:800px;margin:0 auto;text-align:center;position:relative;z-index:10">
<div style="background:rgba(26,86,219,.08);border:1px solid rgba(26,86,219,.15);border-radius:20px;padding:48px 40px">
<div style="font-size:28px;font-weight:800;color:white;margin-bottom:12px">Ready to stop overpaying?</div>
<div style="font-size:16px;color:rgba(255,255,255,.5);margin-bottom:28px">Upload your first invoice and see AuditLens in action.</div>
<button class="bt bt-p" style="padding:14px 32px;font-size:16px;font-weight:700" data-go>Get Started Free \u2192</button>
</div></section>

<footer style="padding:36px 56px;border-top:1px solid rgba(255,255,255,.06);display:flex;justify-content:space-between;align-items:center;position:relative;z-index:10"><div style="font-size:13px;color:rgba(255,255,255,.3);font-weight:500">\u00A9 2026 AuditLens</div><div style="font-size:13px;color:rgba(255,255,255,.3)">Enterprise AP Automation</div></footer></div>`}
...
${S.authUser?`<div style="padding:12px 20px;border-top:1px solid var(--bd)"><div style="display:flex;align-items:center;gap:10px;margin-bottom:8px"><div style="width:34px;height:34px;border-radius:50%;background:var(--ac);color:white;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px">${(S.authUser.name||'?')[0].toUpperCase()}</div><div><div style="font-size:13px;font-weight:700">${E(S.authUser.name)}</div><div style="font-size:11px;color:var(--td)">${E(S.authUser.email)}</div></div></div><div style="display:flex;gap:6px;align-items:center"><span class="bg bg-s" style="font-size:12px">${{analyst:'AP Analyst',manager:'AP Manager',vp:'VP Finance',cfo:'CFO'}[S.authUser.role]||S.authUser.role}</span><button class="bt bt-sm bt-g" data-act="logout" style="margin-left:auto;font-size:11px">Logout</button></div></div>`
:`<div style="padding:12px 20px;border-top:1px solid var(--bd)"><div style="font-size:12px;color:var(--td);font-weight:700;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px">\u{1F464} Active Role</div>
<select id="role-select" style="width:100%;padding:8px 10px;border-radius:8px;border:1px solid var(--bd);background:var(--bg);color:var(--fg);font-size:13px;font-weight:600;cursor:pointer">
<option value="analyst" ${S.currentRole==='analyst'?'selected':''}>AP Analyst ($10K)</option>
<option value="manager" ${S.currentRole==='manager'?'selected':''}>AP Manager ($100K)</option>
<option value="vp" ${S.currentRole==='vp'?'selected':''}>VP Finance ($500K)</option>
<option value="cfo" ${S.currentRole==='cfo'?'selected':''}>CFO (Unlimited)</option>
</select><div style="margin-top:6px"><button class="bt bt-sm bt-p" style="width:100%;font-size:12px" data-act="go-login">\u{1F512} Login / Register</button></div></div>`}
<div style="padding:14px 20px;border-top:1px solid var(--bd)"><div style="display:flex;flex-direction:column;gap:6px"><div style="display:flex;align-items:center;gap:8px"><div style="width:7px;height:7px;border-radius:50%;${S.api==='claude_api'?'background:var(--ok)':'background:var(--wn)'}"></div><span style="font-size:12px;font-weight:500;color:var(--td)">${S.api==='claude_api'?'AI Extraction Active':'Sandbox Mode'}</span></div><div style="display:flex;align-items:center;gap:8px"><div style="width:7px;height:7px;border-radius:50%;${S.dash.db_backend==='postgres'?'background:var(--ok)':'background:var(--wn)'}"></div><span style="font-size:12px;font-weight:500;color:var(--td)">${S.dash.db_backend==='postgres'?'Postgres Connected':'Local Storage'}</span></div></div></div></aside>
<main class="mc">${S.tab==='dashboard'?dashboard(d,ag):S.tab==='triage'?triageTab():S.tab==='documents'?documents():S.tab==='anomalies'?anomaliesTab():S.tab==='matching'?matching():S.tab==='vendors'?vendorsTab():S.tab==='contracts'?contractsTab():S.tab==='settings'?settingsTab():upload()}</main></div>`}
...
<div class="uz" id="uz"><div style="font-size:44px;margin-bottom:16px">${I.up}</div><div style="font-size:17px;font-weight:700;margin-bottom:8px">Drop files here or click to upload</div><div style="font-size:14.5px;color:var(--td);margin-bottom:16px">PDF, JPEG, PNG \u2014 scanned docs supported</div>
