...
let S={scr:'landing',tab:'dashboard',docs:[],matches:[],anomalies:[],dash:{},proc:null,sel:null,toast:null,ut:'auto',api:'unknown',editing:false,editFields:{}};
...
<div style="display:flex;gap:14px;margin-bottom:20px;flex-wrap:wrap">${statCard(I.doc,'Documents',d.total_documents||0,`${d.invoice_count||0} INV \u00B7 ${d.po_count||0} PO \u00B7 ${d.contract_count||0} CTR`,'var(--pp)')}${statCard(I.sp,'Avg Confidence',`${d.avg_confidence||0}%`,d.verified_count?`${d.verified_count} manually verified`:'Extraction accuracy','var(--al)')}${statCard(I.cl,'Due in 7 Days',d.due_in_7_days||0,d.due_in_7_days_amount?fmt(d.due_in_7_days_amount):'\u2014','var(--wn)')}${statCard(I.zp,'Feedback Loop',d.correction_patterns||0,d.correction_patterns?'patterns learned for AI':'Corrections improve AI','var(--ok)')}</div>
...
${eField('Subtotal','subtotal',it.subtotal)}
...
${it.earlyPaymentDiscount?`<div style="margin-bottom:12px;padding:8px 12px;background:rgba(16,185,129,.08);border:1px solid rgba(16,185,129,.15);border-radius:8px;font-size:11px;color:var(--ok);font-weight:500">${I.zp} Early payment: ${it.earlyPaymentDiscount.discount_percent}% off within ${it.earlyPaymentDiscount.days} days (save ${fmtx(it.amount*(it.earlyPaymentDiscount.discount_percent/100),cur)})</div>`:''}
