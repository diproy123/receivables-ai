...
let S={scr:'landing',tab:'dashboard',docs:[],matches:[],anomalies:[],dash:{},vendors:[],triageData:{},proc:null,sel:null,toast:null,ut:'auto',api:'unknown',editing:false,editFields:{},modalView:'split',currentRole:'analyst',policy:{},policyHistory:[],
...
async function loadAll(){
  const[d,dc,m,an,vnd,tri,pol,cas,polH]=await Promise.all([api('/api/dashboard'),api('/api/documents'),api('/api/matches'),api('/api/anomalies'),api('/api/vendors'),api('/api/triage'),api('/api/policy'),api('/api/cases'),api('/api/policy/history')]);
  ...
  if(polH)S.policyHistory=polH.history||[];
...
function logout(){S.authToken=null;S.authUser=null;S.scr='login';S.authMode='login';S.authError=null;sessionStorage.removeItem('al_token');sessionStorage.removeItem('al_user');R();toast('Logged out')}
...
function R(){const sbc=document.querySelector('.sb-nav');if(sbc)S._sbs=sbc.scrollTop;const mc=document.querySelector('.mc');if(mc)S._mcs=mc.scrollTop;document.getElementById('app').innerHTML=`<style>${CSS}</style>`+(S.scr==='login'?loginScreen():S.scr==='landing'?landing():app());bindEvents();requestAnimationFrame(()=>{requestAnimationFrame(()=>{const sbc2=document.querySelector('.sb-nav');if(sbc2&&S._sbs)sbc2.scrollTop=S._sbs;const mc2=document.querySelector('.mc');if(mc2&&S._keepScroll&&S._mcs)mc2.scrollTop=S._mcs;S._keepScroll=false})})}
...
.lp-s{padding:48px 48px 0;max-width:1120px;margin:0 auto;position:relative;z-index:10}
.lp-nav{padding:16px 48px;display:flex;align-items:center;justify-content:space-between;position:relative;z-index:10}
.lp-hero-grid{display:grid;grid-template-columns:1fr 1fr;gap:48px;align-items:center}
.lp-stats-row{display:flex;gap:24px;font-size:13px;color:var(--td)}
...
.lp-grid2d{display:grid;grid-template-columns:1fr 1fr;gap:40px;align-items:center;border-radius:16px;padding:40px}
.lp-lang5{display:grid;grid-template-columns:repeat(5,1fr);gap:6px}
.lp-strip{display:flex;align-items:center;justify-content:space-between;gap:16px;border-radius:12px;padding:24px 32px}
.lp-strip-flow{display:flex;gap:8px;align-items:center;flex-shrink:0}
.lp-ft{padding:28px 48px;border-top:1px solid var(--bd);display:flex;justify-content:space-between;align-items:center;position:relative;z-index:10}
.lp-cta-s{padding:48px 48px 56px;max-width:800px;margin:0 auto;text-align:center;position:relative;z-index:10}
.lp-cta-box{border-radius:20px;padding:40px 36px}
.lp-sec-title{font-size:28px;font-weight:800;color:var(--tx);letter-spacing:-.03em}
.lp-big-text{font-size:24px;font-weight:800;color:white;margin-bottom:14px;letter-spacing:-.02em;line-height:1.2}
@media(max-width:800px){
.lp-s{padding:24px 16px 0}
.lp-nav{padding:12px 16px}
.lp-hero-grid{grid-template-columns:1fr;gap:24px}
.lp-hero-grid h1{font-size:30px}
.lp-hero-pipe{display:none}
.lp-stats-row{gap:12px;flex-wrap:wrap}
.lp-stats-row>div{min-width:70px}
.lp-grid2{grid-template-columns:1fr;gap:12px}
.lp-grid3{grid-template-columns:1fr;gap:12px}
.lp-grid2d{grid-template-columns:1fr;gap:20px;padding:24px 20px}
.lp-lang5{grid-template-columns:repeat(5,1fr);gap:4px}
.lp-strip{flex-direction:column;gap:14px;padding:20px 16px;text-align:center}
.lp-strip-flow{flex-direction:column;gap:6px}
.lp-strip-flow svg{transform:rotate(90deg)}
.lp-ft{padding:16px;flex-direction:column;gap:6px;text-align:center}
.lp-cta-s{padding:24px 16px 36px}
.lp-cta-box{padding:28px 20px}
.lp-sec-title{font-size:22px}
.lp-big-text{font-size:20px}
}`;
...
return`
<div style="min-height:100vh;background:var(--bg);position:relative;overflow-x:hidden">
...
<div style="display:flex;gap:12px;margin-bottom:28px;flex-wrap:wrap"><button class="bt bt-p" style="padding:12px 28px;font-size:15px;font-weight:700" data-go>Start Auditing \u2192</button><button class="bt bt-s" style="padding:12px 28px;font-size:15px" onclick="window.open('mailto:sales@auditlens.ai')">Talk to Sales</button></div>
...
<div class="lp-hero-pipe" style="position:relative">
...
<!-- PROBLEM -->
<section class="lp-s">
<div class="lp-grid2d" style="background:linear-gradient(135deg,#0f172a,#1e293b)">
<div>
...
<div class="lp-big-text">AP teams lose 1\u20143% of spend to undetected overcharges, duplicates, and contract violations.</div>
...
<div class="lp-grid2" style="gap:12px">
${[
...
<div class="lp-sec-title">AI where it matters. Rules where you need control.</div>
...
<div class="lp-grid3" style="margin-bottom:16px">
${[
...
<div class="lp-sec-title">Every number verified. Every decision auditable.</div>
</div>
<div class="lp-grid2">
...
<!-- GLOBAL -->
<section class="lp-s">
<div class="lp-grid2d" style="background:linear-gradient(135deg,#0f172a,#1e293b);padding:32px 40px;gap:36px">
<div>
...
<div class="lp-sec-title" style="color:white;margin-bottom:10px">Process invoices in ${lc} languages</div>
...
<div class="lp-sec-title">Your data. Your infrastructure. Your rules.</div>
<div style="font-size:14px;color:var(--tm);margin-top:8px;max-width:620px;margin-left:auto;margin-right:auto">AuditLens is model-agnostic by design. Choose the deployment that matches your security posture &mdash; from managed cloud to fully air-gapped on-premise.</div>
</div>
<div class="lp-grid3" style="margin-bottom:20px">
${[
{icon:I2.cloud,title:'Managed Cloud',desc:'Anthropic API with Zero Data Retention. Fastest setup &mdash; add API key and go. Your data is never used for training.',tag:'DEFAULT',tagC:'#1a56db',border:'#bfdbfe',bg:'linear-gradient(135deg,#fafafe,#eff6ff)',checks:['Zero Data Retention','Frontier model accuracy','5-minute setup','SOC 2 Type II compliant'],prov:'Anthropic Claude API'},
...
<!-- CTA -->
<section class="lp-cta-s">
<div class="lp-cta-box" style="background:linear-gradient(135deg,var(--ag),#eff6ff);border:1px solid rgba(26,86,219,.12)">
<div class="lp-sec-title" style="margin-bottom:10px">See what your AP team is missing.</div>
...
<footer class="lp-ft"><div style="font-size:12px;color:var(--td);font-weight:500">\u00A9 2026 AuditLens${ver?' \u00B7 v'+ver:''}</div><div style="font-size:12px;color:var(--td)">Enterprise AP Automation \u00B7 SOX-Ready</div></footer></div>`}
...
<div class="sb-nav" style="padding:0 12px;flex:1;overflow-y:auto;min-height:0">
...
<div style="display:flex;gap:10px;margin-bottom:24px;flex-wrap:wrap">${Object.entries(presetInfo).map(([k,v])=>{const active=mm===POLICY_PRESETS_MAP[k];return`<button class="bt bt-sm ${active?'bt-p':'bt-g'}" data-preset="${k}" style="padding:10px 16px"><div style="font-size:15px">${v.i}</div><div style="font-size:13px;font-weight:700">${v.n}</div><div style="font-size:11px;color:${active?'rgba(255,255,255,.85)':'var(--td)'};margin-top:2px">${v.d}</div></button>`}).join('')}</div>
...
${row('Early Payment Discount Detection','early_payment_discount_flag','toggle')}
...
<div style="display:flex;gap:10px"><button class="bt bt-p" data-act="save-policy">\u2714 Save Policy</button><button class="bt bt-g" data-act="reset-policy">\u21BA Reset to Default</button></div>
<div class="cd" style="padding:20px;margin-top:16px"><div style="font-size:14px;font-weight:700;color:var(--tm);margin-bottom:14px;text-transform:uppercase;letter-spacing:.06em">\uD83D\uDCDD Policy Change History</div>
${renderPolicyHistory()}
</div></div>`}
function renderPolicyHistory(){
  const logs=(S.policyHistory||[]).slice(0,15);
  if(!logs.length)return'<div style="font-size:13px;color:var(--td);padding:8px 0">No policy changes recorded yet.</div>';
  return logs.map(function(l){
    var d=new Date(l.timestamp);
    var when=d.toLocaleDateString('en-US',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'});
    var who=E(l.performedBy||'System');
    if(l.action==='policy_preset_applied'){
      return'<div style="padding:10px 14px;background:var(--sa);border-radius:8px;margin-bottom:6px;border-left:3px solid var(--ac)"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:4px"><div style="font-size:13px;font-weight:600;color:var(--tx)">\uD83C\uDFAF Preset applied: <span style="color:var(--ac)">'+E(l.preset)+'</span></div><div style="font-size:11px;color:var(--td)">'+who+' \u00B7 '+when+'</div></div></div>';
    }
    var changes=l.changes||{};var keys=Object.keys(changes);
    if(!keys.length){
      return'<div style="padding:10px 14px;background:var(--sa);border-radius:8px;margin-bottom:6px;border-left:3px solid var(--ok)"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:4px"><div style="font-size:13px;font-weight:600;color:var(--tx)">Policy updated: '+E((l.fieldsChanged||[]).join(', '))+'</div><div style="font-size:11px;color:var(--td)">'+who+' \u00B7 '+when+'</div></div></div>';
    }
    var rows=keys.map(function(k){
      return'<div style="font-size:12px;color:var(--tm);display:flex;gap:6px;align-items:center;flex-wrap:wrap"><span style="font-weight:600;color:var(--tx);font-family:var(--mn);font-size:11px">'+E(k)+'</span><span style="color:var(--dg);text-decoration:line-through;font-family:var(--mn);font-size:11px">'+E(String(changes[k].old))+'</span><span style="color:var(--td)">\u2192</span><span style="color:var(--ok);font-weight:600;font-family:var(--mn);font-size:11px">'+E(String(changes[k].new))+'</span></div>';
    }).join('');
    return'<div style="padding:10px 14px;background:var(--sa);border-radius:8px;margin-bottom:6px;border-left:3px solid var(--ok)"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:4px;margin-bottom:6px"><div style="font-size:13px;font-weight:600;color:var(--tx)">Policy updated</div><div style="font-size:11px;color:var(--td)">'+who+' \u00B7 '+when+'</div></div><div style="display:flex;flex-direction:column;gap:3px">'+rows+'</div></div>';
  }).join('');
}
...
function bindEvents(){
  ...
  document.querySelectorAll('[data-tab]').forEach(e=>e.onclick=async()=>{S.tab=e.dataset.tab;S._keepScroll=false;R();
  ...
  document.querySelectorAll('[data-go]').forEach(e=>e.onclick=async()=>{if(S.authToken){S.scr='app';R();await loadAll()}else{S.authMode='login';S.scr='login';R()}});
  ...
  document.querySelectorAll('[data-act="go-login"]').forEach(e=>e.onclick=()=>{S.authMode='login';S.scr='login';R()});
  // Policy settings bindings
  document.querySelectorAll('[data-preset]').forEach(e=>e.onclick=async()=>{await api(`/api/policy/preset/${e.dataset.preset}`,{method:'POST'});S._keepScroll=true;await loadAll();toast(`Applied ${e.dataset.preset} preset`)});
  document.querySelectorAll('[data-act="save-policy"]').forEach(e=>e.onclick=async()=>{
    ...
    await api('/api/policy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(updates)});S._keepScroll=true;await loadAll();toast('Policy saved')});
  document.querySelectorAll('[data-act="reset-policy"]').forEach(e=>e.onclick=async()=>{await api('/api/policy/preset/enterprise_default',{method:'POST'});S._keepScroll=true;await loadAll();toast('Policy reset to default')});
  document.querySelectorAll('[data-policy-toggle]').forEach(e=>e.onclick=async()=>{const k=e.dataset.policyToggle;const nv=!(S.policy||{})[k];await api('/api/policy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({[k]:nv})});S._keepScroll=true;await loadAll();toast(`${k}: ${nv?'ON':'OFF'}`)});
