...
.bg{display:inline-flex;align-items:center;gap:4px;padding:3px 10px;border-radius:6px;font-size:11px;font-weight:600;letter-spacing:.04em;text-transform:uppercase}.bg-d{background:var(--sa);color:var(--tm)}.bg-s{background:var(--okb);color:var(--ok)}.bg-w{background:var(--wnb);color:var(--wn)}.bg-e{background:var(--dgb);color:var(--dg)}.bg-i{background:var(--ag);color:var(--al)}.bg-p{background:var(--ppb);color:var(--pp)}
.bt{display:inline-flex;align-items:center;gap:8px;padding:10px 22px;border-radius:10px;border:none;cursor:pointer;font-size:14px;font-weight:600;font-family:var(--ft);transition:all .2s}.bt-p{background:var(--ac);color:#fff;box-shadow:0 1px 3px rgba(37,99,235,.3)}.bt-p:hover{background:#1d4ed8;transform:translateY(-1px)}.bt-s{background:var(--sf);color:var(--tx);border:1px solid var(--bd)}.bt-s:hover{background:var(--sa)}.bt-g{background:transparent;color:var(--tm);border:1px solid var(--bd)}.bt-g:hover{background:var(--sa)}.bt-sm{padding:6px 14px;font-size:12px;border-radius:8px}
...
.sbi{display:flex;align-items:center;gap:10px;width:100%;padding:10px 14px;border-radius:10px;border:none;cursor:pointer;font-size:14px;font-weight:500;font-family:var(--ft);margin-bottom:2px;text-align:left;background:transparent;color:var(--tm);transition:all .15s}.sbi:hover{background:var(--sa)}.sbi.on{background:var(--ag);color:var(--al);font-weight:600}
.tw{border-radius:12px;overflow-x:auto;border:1px solid var(--bd);background:var(--sf)}table{width:100%;border-collapse:collapse;font-size:13px;white-space:nowrap}thead tr{background:var(--sa)}th{padding:12px 16px;text-align:left;color:var(--td);font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:.06em}td{padding:12px 16px}tbody tr{border-top:1px solid var(--bd);cursor:pointer;transition:background .15s}tbody tr:hover{background:var(--sa)}
.mn{font-family:var(--mn);font-size:12.5px}.fb{font-weight:700}
...
<div style="display:flex;align-items:center;gap:14px"><img src="${LOGO_URL}" width="64" height="64" style="border-radius:14px"><div><span style="font-size:24px;font-weight:800;letter-spacing:-.03em;background:linear-gradient(135deg,#e53e3e,#f56565);-webkit-background-clip:text;-webkit-text-fill-color:transparent">AuditLens</span><div style="font-size:10.5px;color:var(--td);font-weight:600;text-transform:uppercase;letter-spacing:.1em">AI Eyes. Zero Leakage.</div></div></div>
...
<h1 style="font-size:48px;font-weight:800;letter-spacing:-.04em;line-height:1.1;margin-bottom:20px;color:#1a1d23">Stop overpaying vendors.<br>Start saving money.</h1>
...
<footer style="padding:48px;text-align:center;border-top:1px solid var(--bd)"><div style="font-size:12px;color:var(--td)">Powered by Claude AI \u00B7 Enterprise-ready</div></footer></div>`}
...
<div style="display:flex;align-items:center;gap:12px;padding:0 20px;margin-bottom:32px"><img src="${LOGO_URL}" width="52" height="52" style="border-radius:12px"><div><div style="font-size:18px;font-weight:800;letter-spacing:-.02em;background:linear-gradient(135deg,#e53e3e,#f56565);-webkit-background-clip:text;-webkit-text-fill-color:transparent">AuditLens</div><div style="font-size:9.5px;color:var(--td);font-weight:600;text-transform:uppercase;letter-spacing:.08em">AI Eyes. Zero Leakage.</div></div></div>
...
<div><div style="font-size:13px;opacity:.85;font-weight:500;margin-bottom:4px">\u2714 Total Savings Discovered</div><div style="font-size:36px;font-weight:800;letter-spacing:-.03em;line-height:1.1">${fmt(sv)}</div><div style="font-size:12px;opacity:.75;margin-top:6px">Across ${d.total_documents||0} documents processed</div></div>
<div style="display:flex;gap:20px;flex-wrap:wrap">${sb.overcharges?`<div style="text-align:center"><div style="font-size:18px;font-weight:700">${fmt(sb.overcharges)}</div><div style="font-size:10.5px;opacity:.75">Overcharges</div></div>`:''
}${sb.duplicates_prevented?`<div style="text-align:center"><div style="font-size:18px;font-weight:700">${fmt(sb.duplicates_prevented)}</div><div style="font-size:10.5px;opacity:.75">Duplicates</div></div>`:''
}${sb.contract_violations?`<div style="text-align:center"><div style="font-size:18px;font-weight:700">${fmt(sb.contract_violations)}</div><div style="font-size:10.5px;opacity:.75">Contract Violations</div></div>`:''
}${sb.unauthorized_items?`<div style="text-align:center"><div style="font-size:18px;font-weight:700">${fmt(sb.unauthorized_items)}</div><div style="font-size:10.5px;opacity:.75">Unauthorized</div></div>`:''
}${sb.early_payment_opportunities?`<div style="text-align:center"><div style="font-size:18px;font-weight:700">${fmt(sb.early_payment_opportunities)}</div><div style="font-size:10.5px;opacity:.75">Early Pay Savings</div></div>`:''
}</div></div>`:''}
...
<div><div style="font-size:13px;opacity:.85;font-weight:500;margin-bottom:4px">\u26A1 Processing Speed</div><div style="font-size:32px;font-weight:800;letter-spacing:-.03em;line-height:1.1">${spd}s <span style="font-size:16px;opacity:.75">avg per document</span></div><div style="font-size:12px;opacity:.75;margin-top:4px">vs 15 min manual \u2014 <b>${sfactor}x faster</b></div></div>
<div style="display:flex;gap:20px;flex-wrap:wrap">
<div style="text-align:center"><div style="font-size:16px;font-weight:700">${sp.avg_extraction_ms||0}ms</div><div style="font-size:10.5px;opacity:.75">AI Extraction</div></div>
<div style="text-align:center"><div style="font-size:16px;font-weight:700">${sp.avg_matching_ms||0}ms</div><div style="font-size:10.5px;opacity:.75">PO Matching</div></div>
<div style="text-align:center"><div style="font-size:16px;font-weight:700">${sp.avg_anomaly_ms||0}ms</div><div style="font-size:10.5px;opacity:.75">Anomaly Detection</div></div>
<div style="text-align:center"><div style="font-size:16px;font-weight:700">${sp.avg_triage_ms||0}ms</div><div style="font-size:10.5px;opacity:.75">Triage</div></div>
</div></div>`:''}
...
${ed?'<div style="padding:6px 24px;background:#eff6ff;border-bottom:1px solid #bfdbfe;font-size:12px;color:var(--al);font-weight:500;flex-shrink:0">\u270F\uFE0F Edit mode \u2014 click any highlighted field to correct. Save will re-run anomaly detection.</div>':''}
...
${it.confidenceFactors?`<div style="margin:0 0 8px 0;padding:8px 12px;background:var(--sa);border:1px solid var(--bd);border-radius:8px"><div style="font-size:10px;font-weight:700;color:var(--td);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Confidence Breakdown</div>${Object.entries(it.confidenceFactors).map(([k,v])=>`<div style="display:flex;justify-content:space-between;align-items:center;padding:4px 0;font-size:11px;border-bottom:1px solid var(--bd)"><span style="color:var(--tm)">${k.replace(/_/g,' ')}<span style="color:var(--td);font-size:10px;margin-left:4px">(${Math.round(v.weight*100)}%)</span></span><div style="display:flex;align-items:center;gap:6px"><div style="width:60px;height:4px;background:var(--bd);border-radius:2px;overflow:hidden"><div style="width:${v.score}%;height:100%;background:${v.score>=80?'var(--ok)':v.score>=50?'var(--wn)':'var(--dg)'};border-radius:2px"></div></div><span class="mn fb" style="color:${v.score>=80?'var(--ok)':v.score>=50?'var(--wn)':'var(--dg)'};min-width:24px;text-align:right">${v.score}</span></div></div>`).join('')}</div>`:''}
...
${it.triageLane&&it.triageReasons?`<div style="margin-bottom:12px;padding:10px 14px;background:${it.triageLane==='AUTO_APPROVE'?'#ecfdf5':it.triageLane==='BLOCK'?'#fef2f2':'#fffbeb'};border:1px solid ${it.triageLane==='AUTO_APPROVE'?'#a7f3d0':it.triageLane==='BLOCK'?'#fecaca':'#fde68a'};border-radius:10px"><div style="display:flex;align-items:center;gap:6px;margin-bottom:6px"><span class="bg ${laneBg(it.triageLane)}">${laneLabel(it.triageLane)}</span><span style="font-size:11px;color:var(--td)">AI Triage \u00B7 ${Math.round(it.triageConfidence||0)}% confidence</span></div>${(it.triageReasons||[]).map(r=>`<div style="font-size:12px;color:var(--tm);line-height:1.6;padding:1px 0">\u2022 ${E(r)}</div>`).join('')}</div>`:''}
${it.earlyPaymentDiscount?`<div style="margin-bottom:12px;padding:8px 12px;background:#ecfdf5;border:1px solid #a7f3d0;border-radius:8px;font-size:12px;color:var(--ok);font-weight:500">${I.zp} Early payment: ${it.earlyPaymentDiscount.discount_percent}% off within ${it.earlyPaymentDiscount.days} days (save ${fmtx((it.subtotal||it.amount)*(it.earlyPaymentDiscount.discount_percent/100),cur)})</div>`:''}
...
${docAnoms.map(a=>`<div style="padding:8px 12px;background:#fef2f2;border:1px solid #fecaca;border-radius:8px;margin-bottom:4px;font-size:12px"><div style="display:flex;justify-content:space-between;margin-bottom:3px"><span class="bg ${sevBg(a.severity)}" style="font-size:10px">${a.severity} \u00B7 ${(a.type||'').replace(/_/g,' ')}</span><span class="mn fb" style="color:var(--dg);font-size:12px">${fmtx(Math.abs(a.amount_at_risk||0),cur)}</span></div><div style="color:var(--tm);line-height:1.5;font-size:12px">${E(a.description)}</div></div>`).join('')}</div>`:''}
