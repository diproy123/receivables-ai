...
async function loadPlatformStats(){
  ...
  if(h&&h.stats){const s=h.stats;
    S.ps={v:h.version||'',rc:s.anomaly_rule_count||0,rules:s.anomaly_rules||[],opp:s.opportunity_flags||[],
      ...
      mp:s.models?.primary||'',ms:s.models?.secondary||'',ml:s.models?.primary?s.models.primary+' + '+s.models.secondary:''};
  }
}
  R();
}
...
{i:'\uD83E\uDD16',n:'AI',t:'Ensemble Extraction + Dispute Resolution',d:'Two Claude models ('+(ml||'dual-model')+') extract every field in parallel. A consensus engine merges results with field-level confidence. When models disagree on critical fields, an agentic resolver re-examines the document with vendor history, PO data, and correction patterns \u2014 a 3-step self-correcting pipeline.',tag:'AI-powered \u00B7 '+lc+' languages \u00B7 Self-correcting',color:'#7c3aed'},
