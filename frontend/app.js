...
${(()=>{const logs=(S.policyHistory||[]).slice(0,15);if(!logs.length)return'<div style="font-size:13px;color:var(--td);padding:8px 0">No policy changes recorded yet.</div>';return logs.map(l=>{const d=new Date(l.timestamp);const when=d.toLocaleDateString('en-US',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'});const who=l.performedBy||'System';if(l.action==='policy_preset_applied')return\`<div style="padding:10px 14px;background:var(--sa);border-radius:8px;margin-bottom:6px;border-left:3px solid var(--ac)"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:4px"><div style="font-size:13px;font-weight:600;color:var(--tx)">\uD83C\uDFAF Preset applied: <span style="color:var(--ac)">${l.preset}</span></div><div style="font-size:11px;color:var(--td)">${who} \u00B7 ${when}</div></div></div>\`;const changes=l.changes||{};const keys=Object.keys(changes);if(!keys.length)return\`<div style="padding:10px 14px;background:var(--sa);border-radius:8px;margin-bottom:6px;border-left:3px solid var(--ok)"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:4px"><div style="font-size:13px;font-weight:600;color:var(--tx)">Policy updated: ${(l.fieldsChanged||[]).join(', ')}</div><div style="font-size:11px;color:var(--td)">${who} \u00B7 ${when}</div></div></div>\`;return\`<div style="padding:10px 14px;background:var(--sa);border-radius:8px;margin-bottom:6px;border-left:3px solid var(--ok)"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:4px;margin-bottom:6px"><div style="font-size:13px;font-weight:600;color:var(--tx)">Policy updated</div><div style="font-size:11px;color:var(--td)">${who} \u00B7 ${when}</div></div><div style="display:flex;flex-direction:column;gap:3px">${keys.map(k=>\`<div style="font-size:12px;color:var(--tm);display:flex;gap:6px;align-items:center;flex-wrap:wrap"><span style="font-weight:600;color:var(--tx);font-family:var(--mn);font-size:11px">${k}</span><span style="color:var(--dg);text-decoration:line-through;font-family:var(--mn);font-size:11px">${changes[k].old}</span><span style="color:var(--td)">\u2192</span><span style="color:var(--ok);font-weight:600;font-family:var(--mn);font-size:11px">${changes[k].new}</span></div>\`).join('')}</div></div>\`}).join('')})()}
</div></div>`}
