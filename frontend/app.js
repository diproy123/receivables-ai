...
<p style="font-size:16px;color:var(--tm);line-height:1.7;max-width:480px;margin-bottom:28px">Ensemble AI extraction with agentic dispute resolution. RAG-augmented anomaly detection across ${rc} rules. Three-way matching. Policy-driven triage. End-to-end AP audit automation.</p>
...
${[[rc,'anomaly rules'],[lc,'languages'],['3-way','matching'],['<8s','per doc']].map(([v,l])=>`<div><span style="font-size:20px;font-weight:800;color:var(--ac);letter-spacing:-.02em;display:block;font-family:var(--mn)">${v}</span>${l}</div>`).join('')}
</div>
</div>
...
<div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--td);margin-bottom:16px">\u26A1 Processing Pipeline</div>
${[
...
{icon:'\uD83E\uDD1D',label:'Agentic Dispute Resolution',sub:'Models disagreed on total \u2192 re-examines with vendor + PO context',color:'var(--ac)',bg:'rgba(26,86,219,.08)'},
{icon:'\uD83D\uDD17',label:'3-Way PO-GRN-Invoice Match',sub:'Line items verified against purchase order and goods receipt',color:'#059669',bg:'rgba(5,150,105,.08)'},
{icon:'\uD83E\uDDE0',label:'RAG-Augmented Anomaly Detection',sub:rc+' rules + AI reviews against similar past invoices from vector DB',color:'#d97706',bg:'rgba(217,119,6,.08)'},
{icon:'\u2699\uFE0F',label:'Triage & Case Management',sub:'Auto-approve / review / block routing \u00B7 SLA tracking \u00B7 escalation workflows',color:'#dc2626',bg:'rgba(220,38,38,.08)'},
{icon:'\uD83E\uDDEC',label:'Custom Model Fine-Tuning',sub:'LoRA adapters trained on your corrections \u00B7 gets smarter per vendor',color:'#0891b2',bg:'rgba(8,145,178,.08)'},
].map((s,i)=>`<div style="display:flex;gap:12px;align-items:flex-start;padding:10px 12px;border-radius:10px;background:${s.bg};margin-bottom:6px;animation:fadeUp .4s ease ${.15+i*.12}s both"><div style="width:28px;height:28px;border-radius:7px;background:${s.color}15;display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0">${s.icon}</div><div><div style="font-size:13px;font-weight:700;color:${s.color}">${s.label}</div><div style="font-size:11px;color:var(--tm);margin-top:1px">${s.sub}</div></div></div>`).join('')}
</div>
...
<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:2px;background:var(--bd);border-radius:14px;overflow:hidden;border:1px solid var(--bd)">
${[
{i:'\uD83E\uDD16',t:'Ensemble Extraction',d:(ml||'Dual-model')+' consensus'},
{i:'\uD83E\uDD1D',t:'Dispute Resolution',d:'Agentic self-correction'},
{i:'\uD83E\uDDE0',t:'Anomaly Detection',d:'AI + '+rc+' rules + RAG'},
{i:'\uD83D\uDD17',t:'3-Way Matching',d:'PO \u2194 GRN \u2194 Invoice'},
{i:'\uD83D\uDCCB',t:'Case Management',d:'SLA tracking \u00B7 escalation'},
{i:'\uD83C\uDF10',t:lc+' Languages',d:'Auto-detect \u00B7 locale-aware'},
{i:'\uD83E\uDDEC',t:'Custom Model LoRA',d:'Fine-tune on your data'},
].map(f=>`<div style="background:var(--sf);padding:20px 14px;text-align:center"><div style="font-size:22px;margin-bottom:6px">${f.i}</div><div style="font-size:13px;font-weight:700;color:var(--tx);margin-bottom:3px">${f.t}</div><div style="font-size:11px;color:var(--td);line-height:1.4">${f.d}</div></div>`).join('')}
</div>
</section>
...
<div class="cd" style="padding:20px;margin-bottom:16px"><div style="font-size:14px;font-weight:700;color:var(--tm);margin-bottom:14px;text-transform:uppercase;letter-spacing:.06em">\u23F1\uFE0F Case SLA Targets (hours)</div>
${row('Critical Priority','sla_critical_hours','int')}
${row('High Priority','sla_high_hours','int')}
${row('Medium Priority','sla_medium_hours','int')}
${row('Low Priority','sla_low_hours','int')}</div>
