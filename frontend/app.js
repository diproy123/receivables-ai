...
${[{v:'19',l:'Anomaly Rules'},{v:'3-Way',l:'PO-GRN-Invoice Matching'},{v:'10',l:'Languages Supported'},{v:'<8s',l:'Per Document'}].map(m=>`<div style="background:var(--sf);padding:28px 20px;text-align:center"><div style="font-size:30px;font-weight:800;color:var(--ac);font-family:var(--mn);letter-spacing:-.02em">${m.v}</div><div style="font-size:12px;color:var(--td);font-weight:500;margin-top:6px;text-transform:uppercase;letter-spacing:.06em">${m.l}</div></div>`).join('')}
</div></section>

<section style="padding:0 56px 40px;max-width:1000px;margin:0 auto;position:relative;z-index:10">
<div style="background:linear-gradient(135deg,#eff6ff,#f0fdf4);border:1px solid #bfdbfe;border-radius:16px;padding:32px 40px;text-align:center">
<div style="font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#1e40af;margin-bottom:14px">\uD83C\uDF10 Multilingual Invoice Processing</div>
<div style="font-size:15px;color:#334155;margin-bottom:20px;max-width:640px;margin-left:auto;margin-right:auto">Process invoices from global vendors. AuditLens auto-detects document language, extracts fields with locale-aware number and date parsing, and validates against regional tax systems.</div>
<div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:center">${[['English','EN','#f1f5f9','#334155'],['Chinese \u4E2D\u6587','CN','#fef2f2','#991b1b'],['Japanese \u65E5\u672C\u8A9E','JP','#fff7ed','#9a3412'],['Korean \uD55C\uAD6D\uC5B4','KR','#eff6ff','#1e40af'],['Hindi \u0939\u093F\u0928\u094D\u0926\u0940','IN','#fef9c3','#854d0e'],['German','DE','#f0fdf4','#166534'],['French','FR','#eff6ff','#1e3a8a'],['Spanish','ES','#fef9c3','#713f12'],['Portuguese','BR','#f0fdf4','#14532d'],['Arabic \u0627\u0644\u0639\u0631\u0628\u064A\u0629','SA','#f5f3ff','#5b21b6']].map(([name,code,bg,fg])=>'<div style="padding:8px 16px;border-radius:10px;background:'+bg+';border:1px solid '+fg+'22;display:flex;align-items:center;gap:8px"><span style="font-size:10px;font-weight:700;color:'+fg+';background:'+fg+'15;padding:2px 6px;border-radius:4px;letter-spacing:.04em">'+code+'</span><span style="font-size:13px;font-weight:600;color:'+fg+'">'+name+'</span></div>').join('')}</div>
<div style="margin-top:16px;display:flex;gap:12px;flex-wrap:wrap;justify-content:center;font-size:12px;color:#64748b">${['VAT','GST','MwSt','TVA','IVA','\u589E\u503C\u7A0E','\u6D88\u8CBB\u7A0E','\uBD80\u0020\uAC00\u0020\uAC00\u0020\uCE58\u0020\uC138','ICMS'].map(t=>'<span style="padding:3px 10px;border-radius:6px;background:#f8fafc;border:1px solid #e2e8f0">'+t+'</span>').join('')}</div>
</div></section>
...
{i:'\uD83D\uDEE1\uFE0F',t:'19 Anomaly Detectors',d:'Price overcharges, duplicates, tax fraud, stale invoices, round-number flags, weekend billing, quantity mismatches, and contract violations.'},
...
{i:'\u2699\uFE0F',t:'Configurable Policies',d:'Two-way or three-way matching, tolerance thresholds, and 6 industry presets for manufacturing, services, government, and more.'},
{i:'\uD83C\uDF10',t:'Multilingual Extraction',d:'Process invoices in 10 languages with auto-detected locale, number/date format normalization, and region-specific tax validation.'},
{i:'\uD83D\uDCCB',t:'Case Management',d:'Automatic case creation from anomalies, SLA tracking with auto-escalation, assignment workflow, notes, and resolution audit trail.'},
...
<table class="tb" style="width:100%"><thead><tr><th>Case ID</th><th>Priority</th><th>Title</th><th>Vendor</th><th>Amount</th><th>Status</th><th>SLA</th><th>Assigned</th><th>Created</th><th>Actions</th></tr></thead><tbody>
...
${c.status!=='resolved'&&c.status!=='closed'?`<button class="bt bt-sm bt-g" data-caseaction="resolve" data-caseid="${c.id}" style="font-size:11px;padding:3px 8px">\u2713</button>`:''}
${c.status!=='escalated'&&c.status!=='resolved'&&c.status!=='closed'?`<button class="bt bt-sm bt-g" data-caseaction="escalate" data-caseid="${c.id}" style="font-size:11px;padding:3px 8px">\u26A0</button>`:''}
<button class="bt bt-sm bt-g" data-caseaction="detail" data-caseid="${c.id}" style="font-size:11px;padding:3px 8px">\uD83D\uDD0D</button>
</div></td></tr>`}).join('')}
</tbody></table></div>`:'<div class="cd" style="padding:40px;text-align:center;color:var(--td)">No active cases \u2014 all clear!</div>'}
...
${[{n:'1',t:'Upload',d:'Invoices, POs, contracts, credit/debit notes \u2014 in any language'},{n:'2',t:'AI Extract',d:'Multilingual ensemble AI extracts all fields with locale-aware number & date parsing'},{n:'3',t:'Match & Audit',d:'Auto-match to POs, cross-reference contracts, detect anomalies across 19 rules'},{n:'4',t:'Review & Resolve',d:'See anomalies with risk amounts, resolve or dispute'}].map(s=>`<div style="display:flex;gap:14px;align-items:flex-start;margin-bottom:12px"><div style="width:28px;height:28px;border-radius:8px;flex-shrink:0;background:var(--ag);display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;color:var(--al);font-family:var(--mn)">${s.n}</div><div><div style="font-size:14.5px;font-weight:600">${s.t}</div><div style="font-size:14px;color:var(--td);margin-top:2px">${s.d}</div></div></div>`).join('')}
