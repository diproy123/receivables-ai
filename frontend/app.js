...
${it.confidenceFactors?`<div style="margin:0 0 8px 0;padding:6px 10px;background:var(--sa);border-radius:8px;cursor:pointer" onclick="this.querySelector('.cf-detail').style.display=this.querySelector('.cf-detail').style.display==='none'?'block':'none'"><div style="display:flex;align-items:center;gap:6px"><span style="font-size:10px;font-weight:700;color:var(--td);text-transform:uppercase;letter-spacing:.06em">Confidence Breakdown</span><span style="font-size:9px;color:var(--td)">\u25BC</span></div><div class="cf-detail" style="display:none;margin-top:6px">${Object.entries(it.confidenceFactors).map(([k,v])=>`<div style="display:flex;justify-content:space-between;align-items:center;padding:3px 0;font-size:11px;border-bottom:1px solid var(--bd)"><span style="color:var(--tm)">${k.replace(/_/g,' ')}<span style="color:var(--td);font-size:9px"> (${Math.round(v.weight*100)}%)</span></span><span class="mn fb" style="color:${v.score>=80?'var(--ok)':v.score>=50?'var(--wn)':'var(--dg)'}">${v.score}</span></div>`).join('')}</div></div>`:''}
