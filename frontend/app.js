...
${S.authUser?`<div style="padding:12px 16px;border-top:1px solid var(--bd);flex-shrink:0"><div style="display:flex;align-items:center;gap:10px"><div style="width:32px;height:32px;border-radius:50%;background:var(--ac);color:white;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:13px;flex-shrink:0">${(S.authUser.name||'?')[0].toUpperCase()}</div><div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${E(S.authUser.name)}</div><div style="font-size:11px;color:var(--td);white-space:nowrap;overflow:hidden;text-overflow:ellipsis" title="${E(S.authUser.email)}">${E(S.authUser.email)}</div></div><button class="bt bt-sm bt-g" data-act="logout" style="font-size:11px;padding:4px 10px;flex-shrink:0">Logout</button></div><div style="margin-top:6px"><span class="bg bg-s" style="font-size:11px">${{analyst:'AP Analyst',manager:'AP Manager',vp:'VP Finance',cfo:'CFO'}[S.authUser.role]||S.authUser.role}</span></div></div>`
:`<div style="padding:12px 16px;border-top:1px solid var(--bd);flex-shrink:0"><div style="font-size:11px;color:var(--td);font-weight:600;text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">\u{1F464} Active Role</div>
<select id="role-select" style="width:100%;padding:7px 10px;border-radius:8px;border:1px solid var(--bd);background:var(--bg);color:var(--tx);font-size:12px;font-weight:600;cursor:pointer;font-family:var(--ft)">
...
</select><div style="margin-top:6px"><button class="bt bt-sm bt-p" style="width:100%;font-size:11px;padding:6px" data-act="go-login">\u{1F512} Login / Register</button></div></div>`}
