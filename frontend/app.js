...
${it.confidenceFactors?`<div style="margin:0 0 8px 0;padding:8px 12px;background:var(--sa);border:1px solid var(--bd);border-radius:8px"><div style="font-size:10px;font-weight:700;color:var(--td);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Confidence Breakdown</div>${Object.entries(it.confidenceFactors).map(([k,v])=>`<div style="display:flex;justify-content:space-between;align-items:center;padding:4px 0;font-size:11px;border-bottom:1px solid var(--bd)"><span style="color:var(--tm)">${k.replace(/_/g,' ')}<span style="color:var(--td);font-size:9px;margin-left:4px">(${Math.round(v.weight*100)}%)</span></span><div style="display:flex;align-items:center;gap:6px"><div style="width:60px;height:4px;background:var(--bd);border-radius:2px;overflow:hidden"><div style="width:${v.score}%;height:100%;background:${v.score>=80?'var(--ok)':v.score>=50?'var(--wn)':'var(--dg)'};border-radius:2px"></div></div><span class="mn fb" style="color:${v.score>=80?'var(--ok)':v.score>=50?'var(--wn)':'var(--dg)'};min-width:24px;text-align:right">${v.score}</span></div></div>`).join('')}</div>`:''}
