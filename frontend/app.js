...
const CSS=`:root{--bg:#f4f6f9;--sf:#ffffff;--sa:#f0f2f5;--bd:#e2e6ed;--bh:#d0d5dd;--tx:#1a1d23;--tm:#4b5563;--td:#6b7280;--ac:#2563eb;--al:#2563eb;--ag:rgba(37,99,235,.08);--ok:#059669;--okb:#ecfdf5;--wn:#d97706;--wnb:#fffbeb;--dg:#dc2626;--dgb:#fef2f2;--pp:#7c3aed;--ppb:#f3f0ff;--ft:'Outfit',sans-serif;--mn:'JetBrains Mono',monospace}
...
.bg{display:inline-flex;align-items:center;gap:5px;padding:4px 12px;border-radius:6px;font-size:13px;font-weight:600;letter-spacing:.04em;text-transform:uppercase}.bg-d{background:var(--sa);color:var(--tm)}.bg-s{background:var(--okb);color:var(--ok)}.bg-w{background:var(--wnb);color:var(--wn)}.bg-e{background:var(--dgb);color:var(--dg)}.bg-i{background:var(--ag);color:var(--al)}.bg-p{background:var(--ppb);color:var(--pp)}
.bt{display:inline-flex;align-items:center;gap:8px;padding:10px 22px;border-radius:10px;border:none;cursor:pointer;font-size:15px;font-weight:600;font-family:var(--ft);transition:all .2s}.bt-p{background:var(--ac);color:#fff;box-shadow:0 1px 3px rgba(37,99,235,.3)}.bt-p:hover{background:#1d4ed8;transform:translateY(-1px)}.bt-s{background:var(--sf);color:var(--tx);border:1px solid var(--bd)}.bt-s:hover{background:var(--sa)}.bt-g{background:transparent;color:var(--tm);border:1px solid var(--bd)}.bt-g:hover{background:var(--sa)}.bt-sm{padding:6px 14px;font-size:14px;border-radius:8px}
.cd{background:var(--sf);border:1px solid var(--bd);border-radius:12px;padding:24px;transition:border-color .25s;box-shadow:0 1px 3px rgba(0,0,0,.04)}
.ly{display:flex;min-height:100vh}.sidebar{width:250px;background:var(--sf);border-right:1px solid var(--bd);padding:20px 0;display:flex;flex-direction:column;position:fixed;top:0;left:0;bottom:0;z-index:50;box-shadow:1px 0 4px rgba(0,0,0,.03)}.mc{flex:1;margin-left:250px;padding:32px;overflow-y:auto}
.sbi{display:flex;align-items:center;gap:11px;width:100%;padding:12px 16px;border-radius:10px;border:none;cursor:pointer;font-size:15.5px;font-weight:500;font-family:var(--ft);margin-bottom:3px;text-align:left;background:transparent;color:var(--tm);transition:all .15s}.sbi:hover{background:var(--sa)}.sbi.on{background:var(--ag);color:var(--al);font-weight:600}
.tw{border-radius:12px;overflow-x:auto;border:1px solid var(--bd);background:var(--sf)}table{width:100%;border-collapse:collapse;font-size:15px;white-space:nowrap}thead tr{background:var(--sa)}th{padding:14px 18px;text-align:left;color:var(--td);font-weight:700;font-size:13.5px;text-transform:uppercase;letter-spacing:.06em}td{padding:14px 18px}tbody tr{border-top:1px solid var(--bd);cursor:pointer;transition:background .15s}tbody tr:hover{background:var(--sa)}
.mn{font-family:var(--mn);font-size:14px}.fb{font-weight:700}
...
.mo{position:fixed;inset:0;background:rgba(0,0,0,.4);backdrop-filter:blur(6px);display:flex;align-items:center;justify-content:center;z-index:200}
.po{position:fixed;inset:0;background:rgba(0,0,0,.5);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;z-index:300}
.tt{position:fixed;bottom:28px;left:50%;transform:translateX(-50%);z-index:400;background:var(--sf);border:1px solid rgba(5,150,105,.3);border-radius:12px;padding:12px 24px;font-size:14.5px;font-weight:600;color:var(--ok);display:flex;align-items:center;gap:8px;animation:toastIn .3s ease;box-shadow:0 4px 16px rgba(0,0,0,.12);max-width:600px}
.g2{display:grid;grid-template-columns:1fr 1fr;gap:16px}
.anomaly-card{background:var(--sf);border:1px solid var(--bd);border-radius:12px;padding:16px;margin-bottom:10px;transition:border-color .2s}.anomaly-card:hover{border-color:var(--bh)}
.anomaly-card.sev-high{border-left:3px solid var(--dg)}.anomaly-card.sev-medium{border-left:3px solid var(--wn)}.anomaly-card.sev-low{border-left:3px solid var(--td)}`;
...
<div style="display:flex;align-items:center;gap:14px"><img src="${LOGO_URL}" width="64" height="64" style="border-radius:14px"><div><span style="font-size:26px;font-weight:800;letter-spacing:-.03em;background:linear-gradient(135deg,#e53e3e,#f56565);-webkit-background-clip:text;-webkit-text-fill-color:transparent">AuditLens</span><div style="font-size:12px;color:var(--td);font-weight:600;text-transform:uppercase;letter-spacing:.1em">AI Eyes. Zero Leakage.</div></div></div>
...
<div style="margin-bottom:20px"><img src="${LOGO_URL}" width="110" height="110" style="border-radius:22px;box-shadow:0 12px 40px rgba(229,62,62,.25)"></div>
<div style="display:inline-flex;align-items:center;gap:6px;padding:5px 14px;border-radius:20px;background:var(--ag);border:1px solid rgba(37,99,235,.2);margin-bottom:24px;font-size:14px;font-weight:600;color:var(--al)">${I.zp} AI-Powered Spend Compliance Auditing</div>
<h1 style="font-size:52px;font-weight:800;letter-spacing:-.04em;line-height:1.1;margin-bottom:20px;color:#1a1d23">Stop overpaying vendors.<br>Start saving money.</h1>
<p style="font-size:20px;color:var(--tm);line-height:1.6;max-width:640px;margin:0 auto 36px">AuditLens reads your invoices, cross-references contracts and POs, and catches overcharges, duplicate payments, and compliance violations automatically.</p>
<div style="display:flex;gap:12px;justify-content:center"><button class="bt bt-p" style="padding:14px 32px;font-size:17px" data-go>${I.pl} Launch Demo</button><button class="bt bt-s" style="padding:14px 32px;font-size:17px">Book a Call</button></div></section>
...
${[{v:'$44.9K',l:'Anomalies Found in Test'},{v:'8',l:'Anomaly Types Detected'},{v:'96%',l:'Extraction Accuracy'},{v:'<2sec',l:'Per Document'}].map(m=>`<div style="flex:1;background:var(--sf);padding:24px 20px;text-align:center"><div style="font-size:30px;font-weight:800;color:var(--al);font-family:var(--mn)">${m.v}</div><div style="font-size:13px;color:var(--td);font-weight:500;margin-top:4px;text-transform:uppercase;letter-spacing:.05em">${m.l}</div></div>`).join('')}
</div></section>
...
].map(f=>`<div class="cd" style="padding:28px"><div style="width:40px;height:40px;border-radius:10px;background:var(--ag);display:flex;align-items:center;justify-content:center;color:var(--al);margin-bottom:16px;font-size:22px">${f.i}</div><div style="font-size:17px;font-weight:700;margin-bottom:8px">${f.t}</div><div style="font-size:15px;color:var(--tm);line-height:1.6">${f.d}</div></div>`).join('')}</div></section>
<footer style="padding:48px;text-align:center;border-top:1px solid var(--bd)"><div style="font-size:14px;color:var(--td)">Powered by Claude AI \u00B7 Enterprise-ready</div></footer></div>`}

function statCard(icon,label,val,sub,color){return`<div class="cd" style="flex:1;min-width:180px;padding:26px"><div style="display:flex;align-items:center;gap:10px;margin-bottom:14px"><div style="width:36px;height:36px;border-radius:9px;background:${color}18;display:flex;align-items:center;justify-content:center;color:${color};font-size:18px">${icon}</div><span style="font-size:14px;color:var(--td);font-weight:600;text-transform:uppercase;letter-spacing:.06em">${label}</span></div><div style="font-size:30px;font-weight:700;letter-spacing:-.02em">${val}</div>${sub?`<div style="font-size:14.5px;color:var(--td);margin-top:6px">${sub}</div>`:''}</div>`}
...
<div style="display:flex;align-items:center;gap:12px;padding:0 20px;margin-bottom:32px"><img src="${LOGO_URL}" width="52" height="52" style="border-radius:12px"><div><div style="font-size:20px;font-weight:800;letter-spacing:-.02em;background:linear-gradient(135deg,#e53e3e,#f56565);-webkit-background-clip:text;-webkit-text-fill-color:transparent">AuditLens</div><div style="font-size:12px;color:var(--td);font-weight:600;text-transform:uppercase;letter-spacing:.08em">AI Eyes. Zero Leakage.</div></div></div>
<div style="padding:0 10px;flex:1"><div style="font-size:12px;color:var(--td);font-weight:700;text-transform:uppercase;letter-spacing:.08em;padding:0 12px;margin-bottom:8px">Main</div>
...
<div style="font-size:12px;color:var(--td);font-weight:700;text-transform:uppercase;letter-spacing:.08em;padding:0 12px;margin:20px 0 8px">Admin</div>
<button class="sbi" data-act="seed" style="color:var(--ok)">${I.sp} Seed Demo Data</button>
<button class="sbi" data-act="export">${I.doc} Export DB</button>
<button class="sbi" data-act="import">${I.up} Import DB</button>
<button class="sbi" data-act="reset" style="color:var(--dg)">${I.tr} Reset Data</button></div>
<div style="padding:16px 20px;border-top:1px solid var(--bd)"><div style="display:inline-flex;align-items:center;gap:6px;padding:4px 12px;border-radius:8px;font-size:13px;font-weight:600;${S.api==='claude_api'?'background:var(--okb);color:var(--ok)':'background:var(--wnb);color:var(--wn)'}">${I.sp} ${S.api==='claude_api'?'Claude API Live':'Mock Mode'}</div><div style="display:inline-flex;align-items:center;gap:6px;padding:4px 12px;border-radius:8px;font-size:13px;font-weight:600;margin-top:6px;${S.dash.db_backend==='postgres'?'background:var(--okb);color:var(--ok)':'background:var(--wnb);color:var(--wn)'}">${S.dash.db_backend==='postgres'?'\uD83D\uDFE2 Postgres (persistent)':'\uD83D\uDFE1 File (ephemeral)'}</div></div></aside>
...
function matching(){
  ...
  return`<div class="fu"><div style="margin-bottom:24px"><h1 style="font-size:26px;font-weight:800;margin-bottom:4px">${I.lnk} PO Matching</h1><p style="font-size:15px;color:var(--tm)">${mt} auto-matched \u00B7 ${rv.length} need review</p></div>
...
function triageTab(){
  ...
  return`<div class="fu"><div style="margin-bottom:24px"><h1 style="font-size:26px;font-weight:800;margin-bottom:4px">\u26A1 Agentic Triage</h1><p style="font-size:15px;color:var(--tm)">AI classifies invoices into Auto-Approve, Review, or Block lanes</p></div>
...
function upload(){return`<div class="fu"><div style="margin-bottom:24px"><h1 style="font-size:26px;font-weight:800;margin-bottom:4px">${I.up} Upload Document</h1><p style="font-size:15px;color:var(--tm)">Upload invoices, POs, contracts, or credit/debit notes</p></div>
...
return`<div class="po"><div class="cd" style="padding:44px 52px;text-align:center;max-width:440px;border:1px solid #bfdbfe;box-shadow:0 8px 40px rgba(37,99,235,.15)"><div style="width:56px;height:56px;border-radius:14px;margin:0 auto 20px;background:var(--ac);display:flex;align-items:center;justify-content:center;animation:pulse 1.5s ease-in-out infinite;color:#fff;font-size:22px">${I.sp}</div><div style="font-size:18px;font-weight:700;margin-bottom:6px">Auditing...</div><div style="font-size:14.5px;color:var(--tm);margin-bottom:20px">${E(S.proc.fn)}</div>
<div style="text-align:left">${st.map((s,i)=>`<div style="display:flex;align-items:center;gap:8px;font-size:14px;margin-bottom:6px;color:${i<=cs?'var(--ok)':'var(--td)'}">${i<=cs?I.ck:'\u25CB'} ${s}</div>`).join('')}</div>
<div style="height:3px;background:var(--bd);border-radius:2px;overflow:hidden;margin-top:20px"><div style="height:100%;background:var(--ac);border-radius:2px;animation:loading 1.8s ease-in-out infinite"></div></div></div></div>`}
...
<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap"><span class="bg ${typBg(it.type)}">${typLabel(it.type)}</span><span style="font-size:18px;font-weight:700">${E(it.invoiceNumber||it.poNumber||it.contractNumber||it.documentNumber||it.id)}</span><span class="bg bg-d">${cur}</span>
