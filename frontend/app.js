...
<div style="position:absolute;top:-300px;right:-200px;width:900px;height:900px;border-radius:50%;background:radial-gradient(circle,rgba(26,86,219,.05) 0%,transparent 55%);pointer-events:none"></div>
<div style="position:absolute;bottom:-200px;left:-100px;width:600px;height:600px;border-radius:50%;background:radial-gradient(circle,rgba(220,38,38,.03) 0%,transparent 55%);pointer-events:none"></div>
...
<h1 style="font-size:44px;font-weight:800;letter-spacing:-.04em;line-height:1.08;margin-bottom:18px;color:var(--tx)">Every invoice audited.<br><span style="background:linear-gradient(135deg,var(--ac),#3b82f6);-webkit-background-clip:text;-webkit-text-fill-color:transparent">Every dollar accounted.</span></h1>
<p style="font-size:16px;color:var(--tm);line-height:1.7;max-width:480px;margin-bottom:28px">14 AI enablers across the AP lifecycle. Ensemble extraction with dispute resolution. RAG-augmented anomaly detection across ${rc} rules. AI investigation briefs, smart matching, natural language policy, payment optimization, and vendor intelligence — all grounded in verified data.</p>
...
<div style="display:flex;gap:20px;font-size:13px;color:var(--td)">
${[['14','AI enablers'],[rc,'anomaly rules'],[lc,'languages'],['<8s','per doc']].map(([v,l])=>`<div><span style="font-size:20px;font-weight:800;color:var(--ac);letter-spacing:-.02em;display:block;font-family:var(--mn)">${v}</span>${l}</div>`).join('')}
</div>
</div>
...
<div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--td);margin-bottom:16px">\u26A1 AI-Powered Pipeline — 14 Enablers</div>
${[
{icon:'\uD83E\uDD16',label:'Ensemble AI Extraction',sub:(ml||'Dual-model')+' \u00B7 parallel extract \u00B7 field-level consensus',color:'#7c3aed',bg:'rgba(124,58,237,.08)'},
{icon:'\uD83E\uDD1D',label:'Agentic Dispute Resolution',sub:'Models disagreed? Third AI call re-examines with full context',color:'var(--ac)',bg:'rgba(26,86,219,.08)'},
{icon:'\uD83D\uDD17',label:'3-Way Matching + Smart PO Match',sub:'Exact match + AI fuzzy resolution for unmatched invoices',color:'#059669',bg:'rgba(5,150,105,.08)'},
{icon:'\uD83E\uDDE0',label:'RAG-Augmented Anomaly Detection',sub:rc+' rules + AI auditor with vector DB context injection',color:'#d97706',bg:'rgba(217,119,6,.08)'},
{icon:'\uD83D\uDCCB',label:'AI Investigation Briefs',sub:'Auto-generated case narratives from verified invoice, PO, contract data',color:'#dc2626',bg:'rgba(220,38,38,.08)'},
{icon:'\uD83D\uDCA1',label:'Plain English Anomalies',sub:'One-sentence explanations a finance manager understands',color:'#ea580c',bg:'rgba(234,88,12,.08)'},
{icon:'\u2709\uFE0F',label:'Vendor Communication Drafts',sub:'AI-drafted dispute letters citing exact contract clauses & amounts',color:'#0891b2',bg:'rgba(8,145,178,.08)'},
{icon:'\uD83D\uDCB0',label:'AI Payment Prioritization',sub:'Optimized payment runs: capture EPD, avoid disputed invoices',color:'#16a34a',bg:'rgba(22,163,74,.08)'},
{icon:'\uD83D\uDCCA',label:'Anomaly Pattern Insights',sub:'Statistical patterns + AI synthesis of vendor behavior trends',color:'#9333ea',bg:'rgba(147,51,234,.08)'},
{icon:'\uD83C\uDFAF',label:'Smart Case Routing',sub:'AI-recommended assignments based on expertise + workload + SLA',color:'#0369a1',bg:'rgba(3,105,161,.08)'},
{icon:'\uD83D\uDCAC',label:'Natural Language Policy',sub:'Configure AP rules in plain English \u2014 AI translates to parameters',color:'#c026d3',bg:'rgba(192,38,211,.08)'},
{icon:'\uD83E\uDDEC',label:'Custom Model Fine-Tuning',sub:'LoRA adapters trained on corrections \u00B7 smarter per vendor over time',color:'#475569',bg:'rgba(71,85,105,.08)'},
].map((s,i)=>`<div style="display:flex;gap:12px;align-items:flex-start;padding:8px 12px;border-radius:10px;background:${s.bg};margin-bottom:4px;animation:fadeUp .4s ease ${.15+i*.08}s both"><div style="width:26px;height:26px;border-radius:7px;background:${s.color}15;display:flex;align-items:center;justify-content:center;font-size:13px;flex-shrink:0">${s.icon}</div><div><div style="font-size:12px;font-weight:700;color:${s.color}">${s.label}</div><div style="font-size:10px;color:var(--tm);margin-top:1px">${s.sub}</div></div></div>`).join('')}
</div>
...
<div style="text-align:center;margin-bottom:28px"><div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:var(--ac);margin-bottom:8px">AI Intelligence Layer</div><div style="font-size:28px;font-weight:800;color:var(--tx);letter-spacing:-.03em">14 AI enablers. Zero hallucination architecture.</div><div style="font-size:14px;color:var(--tm);margin-top:8px;max-width:680px;margin-left:auto;margin-right:auto">Every AI output is grounded in verified data from your invoices, POs, and contracts. Claude reasons — the system provides facts — post-validation verifies every number.</div></div>

<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px">
${[
{icon:'\uD83E\uDD16',title:'Ensemble Extraction',desc:'Sonnet + Haiku extract in parallel. Field-level consensus merge. Math validation.',tag:'EXTRACTION',color:'#7c3aed'},
{icon:'\uD83E\uDD1D',title:'Dispute Resolution',desc:'Models disagree? Third AI call re-examines the document with vendor + PO context.',tag:'EXTRACTION',color:'#4f46e5'},
{icon:'\uD83E\uDDEC',title:'Custom Model LoRA',desc:'Fine-tuned on your corrections. Gets faster and cheaper per vendor over time.',tag:'EXTRACTION',color:'#0891b2'},
{icon:'\uD83E\uDDE0',title:'RAG Anomaly Detection',desc:rc+' rules + AI auditor with vector DB context: past anomalies, corrections, contract clauses.',tag:'DETECTION',color:'#d97706'},
{icon:'\uD83D\uDCA1',title:'Plain English Anomalies',desc:'One-sentence explanations a finance manager understands. Post-validated amounts.',tag:'DETECTION',color:'#ea580c'},
{icon:'\uD83D\uDCCA',title:'Pattern Insights',desc:'Statistical analysis + AI synthesis of vendor behavior. Minimum 3 invoices required.',tag:'DETECTION',color:'#9333ea'},
{icon:'\uD83D\uDD17',title:'3-Way + Smart Match',desc:'Exact PO-GRN-Invoice matching plus AI fuzzy resolution for unmatched invoices.',tag:'MATCHING',color:'#059669'},
{icon:'\uD83D\uDCCB',title:'Investigation Briefs',desc:'AI-written case narratives citing exact amounts, contracts, and vendor history.',tag:'CASE MGMT',color:'#dc2626'},
{icon:'\uD83C\uDFAF',title:'Smart Case Routing',desc:'Algorithm scores users by expertise + workload + authority. AI explains the recommendation.',tag:'CASE MGMT',color:'#0369a1'},
{icon:'\u2709\uFE0F',title:'Vendor Comm Drafts',desc:'Dispute letters referencing exact contract clauses and dollar amounts. Human confirms before send.',tag:'CASE MGMT',color:'#0891b2'},
{icon:'\uD83D\uDCB0',title:'Payment Prioritization',desc:'Capture early payment discounts. Hold disputed invoices. Optimize weekly payment runs.',tag:'TREASURY',color:'#16a34a'},
{icon:'\uD83D\uDCAC',title:'Natural Language Policy',desc:'"Be strict on construction" \u2192 AI translates to parameters. Preview before apply.',tag:'CONFIG',color:'#c026d3'},
].map((f,i)=>{const tagColors={'EXTRACTION':'#7c3aed','DETECTION':'#d97706','MATCHING':'#059669','CASE MGMT':'#dc2626','TREASURY':'#16a34a','CONFIG':'#c026d3'};const tc=tagColors[f.tag]||'#6b7493';return`<div style="background:var(--sf);border:1px solid var(--bd);border-radius:12px;padding:18px;animation:fadeUp .4s ease ${.1+i*.05}s both"><div style="display:flex;align-items:center;gap:8px;margin-bottom:8px"><span style="font-size:18px">${f.icon}</span><span style="font-size:10px;font-weight:700;padding:2px 8px;border-radius:4px;background:${tc}10;color:${tc};letter-spacing:.04em">${f.tag}</span></div><div style="font-size:14px;font-weight:700;color:var(--tx);margin-bottom:4px">${f.title}</div><div style="font-size:12px;color:var(--tm);line-height:1.5">${f.desc}</div></div>`}).join('')}
</div>

<div style="background:linear-gradient(135deg,#0f172a,#1e293b);border-radius:14px;padding:24px 30px;display:flex;align-items:center;gap:24px">
<div style="flex:1"><div style="font-size:14px;font-weight:700;color:#93c5fd;margin-bottom:6px">\uD83D\uDEE1\uFE0F Anti-Hallucination Architecture</div><div style="font-size:12px;color:#94a3b8;line-height:1.7">Every AI output passes through: <strong style="color:#e2e8f0">Grounded Context</strong> (all facts from DB) \u2192 <strong style="color:#e2e8f0">Constrained Generation</strong> (scoped prompts) \u2192 <strong style="color:#e2e8f0">Post-Validation</strong> (every $ amount verified) \u2192 <strong style="color:#e2e8f0">Deterministic Fallback</strong> (template-based backup if AI fails).</div></div>
<div style="display:flex;gap:6px;flex-wrap:wrap;max-width:280px">${['Fact Injection','Post-Validation','Fallback','Human Confirm','Provenance Tags','Stat Gating'].map(t=>`<span style="padding:3px 10px;border-radius:6px;background:rgba(96,165,250,.1);border:1px solid rgba(96,165,250,.2);color:#93c5fd;font-size:10px;font-weight:600">${t}</span>`).join('')}</div>
</div>
</section>

<section style="padding:48px 48px 0;max-width:1120px;margin:0 auto;position:relative;z-index:10">
<div style="text-align:center;margin-bottom:28px"><div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:var(--ac);margin-bottom:8px">How It Works</div><div style="font-size:28px;font-weight:800;color:var(--tx);letter-spacing:-.03em">AI where it matters. Rules where you need control.</div></div>
<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:16px">${[
{i:'\uD83E\uDD16',n:'AI',t:'Ensemble Extraction + Dispute Resolution',d:'Two Claude models ('+(ml||'dual-model')+') extract every field in parallel, optionally joined by a custom fine-tuned model as a third extractor. A consensus engine merges results with majority voting and field-level confidence. When models disagree on critical fields, an agentic resolver re-examines the document with vendor history, PO data, and correction patterns \u2014 a self-correcting pipeline.',tag:'AI-powered \u00B7 '+lc+' languages \u00B7 Up to 3-model ensemble',color:'#7c3aed'},
{i:'\uD83E\uDDE0',n:'AI + Rules',t:'RAG-Augmented Anomaly Detection',d:'Claude analyzes each invoice against POs, contracts, and vendor history. A RAG engine retrieves similar past documents from a vector store to enrich the analysis. '+rc+' rule-based detectors run in parallel as a deterministic safety net \u2014 always on, even without API access.',tag:'Hybrid AI+rules \u00B7 Vector search \u00B7 Risk-adjusted',color:'#d97706'},
{i:'\uD83D\uDD17',n:'Algorithmic',t:'Three-Way Matching',d:'Deterministic PO-GRN-Invoice matching verifies quantities, unit prices, and totals across all three documents. Catches overbilling, short shipments, unreceipted invoices, and quantity mismatches with configurable tolerance thresholds.',tag:'Deterministic \u00B7 Configurable tolerances \u00B7 Auditable',color:'#059669'},
{i:'\u2699\uFE0F',n:'Policy Engine',t:'Triage, Authority & Case Management',d:'Invoices are classified into auto-approve, review, or block lanes by a transparent, rule-based engine \u2014 based on anomaly severity, vendor risk scores, match quality, and authority limits. Every decision is explainable and auditable. Cases auto-create with SLA tracking.',tag:'Rule-based \u00B7 SOX-ready \u00B7 Fully explainable',color:'#dc2626'},
{i:'\uD83E\uDDEC',n:'AI + LoRA',t:'Custom Model Fine-Tuning via Together.ai',d:'Human corrections feed a LoRA training pipeline. After 50+ corrections, AuditLens fine-tunes a Qwen 7B model on your vendor-specific extraction patterns via Together.ai \u2014 no GPU needed. The custom model joins the ensemble as a third extractor, weighted by per-vendor accuracy. Retrains monthly, gets better over time.',tag:'LoRA adapters \u00B7 Together.ai \u00B7 Vendor-specific learning',color:'#0891b2'},
].map((f,i,a)=>`<div class="cd" style="padding:26px;animation:fadeUp .5s ease ${i*.1}s both${i===a.length-1?';grid-column:1/-1':''}"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px"><div style="display:flex;align-items:center;gap:10px"><div style="font-size:24px">${f.i}</div><div style="font-size:11px;font-weight:800;color:${f.color};background:${f.color}12;padding:3px 10px;border-radius:6px;letter-spacing:.04em">${f.n}</div></div></div><div style="font-size:16px;font-weight:700;margin-bottom:8px;color:var(--tx)">${f.t}</div><div style="font-size:13px;color:var(--tm);line-height:1.65;margin-bottom:10px">${f.d}</div><div style="font-size:11px;font-weight:600;color:${f.color};padding:5px 10px;background:${f.color}08;border:1px solid ${f.color}20;border-radius:6px;display:inline-block">${f.tag}</div></div>`).join('')}</div>
</section>

<section style="padding:48px 48px 0;max-width:1120px;margin:0 auto;position:relative;z-index:10">
<div style="background:linear-gradient(135deg,#0f172a,#1e293b);border-radius:16px;padding:36px 40px;display:grid;grid-template-columns:1fr 1fr;gap:36px;align-items:center">
<div>
<div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#60a5fa;margin-bottom:10px">\uD83C\uDF10 Global Ready</div>
...
<div style="font-size:14px;color:#94a3b8;line-height:1.6;margin-bottom:16px">Auto-detect document language, normalize regional number and date formats, and validate against local tax systems \u2014 VAT, GST, MwSt, TVA, IVA, and more.</div>
...
<div style="font-size:13px;font-weight:700;color:#1d4ed8;text-transform:uppercase;letter-spacing:.06em;margin-bottom:10px">\uD83D\uDEE1\uFE0F ${rc} Anomaly Rules${oppNames.length?' + '+oppNames.length+' Opportunity Flag'+(oppNames.length>1?'s':''):''}</div>
<div style="display:flex;flex-wrap:wrap;gap:4px">${ruleNames.map(r=>`<span style="font-size:10px;padding:2px 8px;border-radius:4px;background:white;border:1px solid #dbeafe;color:#1e40af;font-weight:500">${r}</span>`).join('')}${oppNames.map(r=>`<span style="font-size:10px;padding:2px 8px;border-radius:4px;background:#ecfdf5;border:1px solid #a7f3d0;color:#059669;font-weight:500">\u2713 ${r}</span>`).join('')}</div>
</div>
<div class="cd" style="padding:24px;background:linear-gradient(135deg,#fef2f2,#fff1f2);border:1px solid #fecaca">
<div style="font-size:13px;font-weight:700;color:#dc2626;text-transform:uppercase;letter-spacing:.06em;margin-bottom:10px">\uD83D\uDCCB Case Lifecycle</div>
<div style="font-size:12px;color:#991b1b;line-height:2">${['Open \u2192 Investigating \u2192 Pending Vendor','Pending Approval \u2192 Resolved \u2192 Closed','Escalated \u2191 (SLA breach auto-escalation)'].map(l=>`<div style="padding:3px 10px;background:white;border-radius:6px;border:1px solid #fecaca;margin-bottom:3px;font-weight:500">${l}</div>`).join('')}
<div style="margin-top:6px;font-size:11px;color:#b91c1c">SLA: Critical ${sla.critical||4}h \u00B7 High ${sla.high||24}h \u00B7 Medium ${sla.medium||72}h \u00B7 Low ${Math.round((sla.low||168)/24)}d</div></div>
</div>
<div class="cd" style="padding:24px;background:linear-gradient(135deg,#f0fdf4,#ecfdf5);border:1px solid #bbf7d0">
<div style="font-size:13px;font-weight:700;color:#16a34a;text-transform:uppercase;letter-spacing:.06em;margin-bottom:10px">\uD83D\uDC65 Delegation of Authority</div>
<div style="font-size:12px">${auth.length?auth.map((a,i)=>`<div style="display:flex;justify-content:space-between;padding:5px 10px;background:white;border-radius:6px;border:1px solid #bbf7d0;margin-bottom:3px"><span style="font-weight:600;color:${authColors[i]||'var(--tm)'}">${a.title}</span><span style="font-family:var(--mn);font-weight:700;color:var(--tm)">${a.unlimited?'Unlimited':'\u2264 $'+a.limit_usd.toLocaleString()}</span></div>`).join(''):[
{r:'AP Analyst',l:'Configurable'},{r:'AP Manager',l:'Configurable'},{r:'VP Finance',l:'Configurable'},{r:'CFO',l:'Unlimited'}
].map((a,i)=>`<div style="display:flex;justify-content:space-between;padding:5px 10px;background:white;border-radius:6px;border:1px solid #bbf7d0;margin-bottom:3px"><span style="font-weight:600;color:${authColors[i]}">${a.r}</span><span style="font-family:var(--mn);font-weight:700;color:var(--tm)">${a.l}</span></div>`).join('')}
<div style="margin-top:6px;font-size:11px;color:#15803d">JWT auth \u00B7 RBAC \u00B7 SOX-ready audit trail</div></div>
</div>
</div>
</section>
...
<div style="font-size:26px;font-weight:800;color:var(--tx);margin-bottom:10px;letter-spacing:-.02em">Ready to stop overpaying?</div>
<div style="font-size:15px;color:var(--tm);margin-bottom:24px">Upload your first invoice and see AuditLens in action \u2014 free.</div>
<button class="bt bt-p" style="padding:13px 28px;font-size:15px;font-weight:700" data-go>Get Started Free \u2192</button>
</div></section>

<footer style="padding:28px 48px;border-top:1px solid var(--bd);display:flex;justify-content:space-between;align-items:center;position:relative;z-index:10"><div style="font-size:12px;color:var(--td);font-weight:500">\u00A9 2026 AuditLens${ver?' \u00B7 v'+ver:''}</div><div style="font-size:12px;color:var(--td)">Enterprise AP Automation</div></footer></div>`}
