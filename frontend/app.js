...
function R(){const sb=document.querySelector('.sidebar');const sst=sb?sb.scrollTop:0;const mc=document.querySelector('.mc');const mst=mc?mc.scrollTop:0;document.getElementById('app').innerHTML=`<style>${CSS}</style>`+(S.scr==='login'?loginScreen():S.scr==='landing'?landing():app());bindEvents();const sb2=document.querySelector('.sidebar');if(sb2)sb2.scrollTop=sst;const mc2=document.querySelector('.mc');if(mc2&&S._keepScroll)mc2.scrollTop=mst;S._keepScroll=false}
...
function renderPolicyHistory(){
  ...
  return logs.map(function(l){
    ...
    if(!keys.length){
      return'<div style="padding:10px 14px;background:var(--sa);border-radius:8px;margin-bottom:6px;border-left:3px solid var(--ok)"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:4px"><div style="font-size:13px;font-weight:600;color:var(--tx)">Policy updated: '+(l.fieldsChanged||[]).join(', ')+'</div><div style="font-size:11px;color:var(--td)">'+who+' \u00B7 '+when+'</div></div></div>';
    }
    var rows=keys.map(function(k){
      return'<div style="font-size:12px;color:var(--tm);display:flex;gap:6px;align-items:center;flex-wrap:wrap"><span style="font-weight:600;color:var(--tx);font-family:var(--mn);font-size:11px">'+k+'</span><span style="color:var(--dg);text-decoration:line-through;font-family:var(--mn);font-size:11px">'+changes[k].old+'</span><span style="color:var(--td)">\u2192</span><span style="color:var(--ok);font-weight:600;font-family:var(--mn);font-size:11px">'+changes[k].new+'</span></div>';
    }).join('');
